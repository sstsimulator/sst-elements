...
Rtlmodel::Rtlmodel(SST::ComponentId_t id, SST::Params& params) :
   ...
   //set our clock
   clock_handler = new Clock::Handler<Rtlmodel,&Rtlmodel::clockTick>(this);
   ...
   ArielRtlLink = configureLink("ArielRtllink", new Event::Handler<Rtlmodel,&Rtlmodel::handleArielEvent>(this));

    // Find all the components loaded into the "memory" slot
    // Make sure all cores have a loaded subcomponent in their slot
    cacheLink = loadUserSubComponent<Interfaces::StandardMem>("memory", ComponentInfo::SHARE_NONE, &timeConverter, new StandardMem::Handler<Rtlmodel,&Rtlmodel::handleMemEvent>(this));
    if(!cacheLink) {
       ...
       cacheLink = loadAnonymousSubComponent<Interfaces::StandardMem>(interfaceName, "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
               interfaceParams, &timeConverter, new StandardMem::Handler<Rtlmodel,&Rtlmodel::handleMemEvent>(this));
