...
public:
        VanadisMemoryMapHandlerState( uint32_t verbosity,
		uint64_t address, uint64_t length, int64_t prot_flg,
		...
		map_address(address), map_length(length),
		map_protect(prot_flg), map_flags(map_flg),
		call_stack(stack_p), offset_units(offset_size),
		memory_mgr(mem_mgr) {
		...
		output->verbose(CALL_INFO, 16, 0, "[syscall-mmap] processing incoming request (addr: 0x%llx, size: %" PRIu64 ")\n",
		...
		case 1:
			{
				...
				output->verbose(CALL_INFO, 16, 0, "[syscall-mmap] -> mmap( 0x%llx, %" PRIu64 ", %" PRId64 ", %" PRId64 ", %" PRId64 ", %" PRIu64 " ) file-offset-units: %" PRIu64 "\n",
					map_address, map_length, map_protect, map_flags, file_descriptor, file_offset, offset_units);
				...
				if( (map_flags & 0x800) != 0x0 ) {
					...
					if( 0 == map_address ) {
						...
						output->verbose(CALL_INFO, 16, 0, "[syscall-mmap] ---> memory manager status: %d / allocation_start: 0x%llx\n",
