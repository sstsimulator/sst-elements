...
//#define DBprintf if (((routerID==446) || (routerID==447) || (routerID==439) || (routerID==431) || (routerID==423) || (routerID==415) || (routerID==407) || (routerID==399)) && cycle() >= 42000) printf
            ...
            m_dbg.output(CALL_INFO, "%" PRId64 ": router %d theQ size %ld rp %p\n", cycle(), routerID, theQ->size(), rp);

            m_dbg.output(CALL_INFO, "%" PRId64 " router %d trying parcel from inQ %d:%d to %d:%d (size %d)\n",
            ...
            if (theQ->empty()) {
                inQ->ready_vcQs--;
                m_dbg.output(CALL_INFO, "%" PRId64 ": router %d inQ %d down to %d ready vcs\n", cycle(), routerID, ilink, inQ->ready_vcQs);
            }
        }
...
//#define DBprintf printf
    for (int i = 0; i < ROUTER_NUM_OUTQS; i++) {
        oLCB = 0;
        int in_queue = 0;
        m_dbg.output(CALL_INFO, "%" PRId64 ": router %d is checking outq %d\n", cycle(), routerID, i);
        //Make sure to push LCB data when possible
        if (outLCB[i].readyXfer()) {
            m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d ready to send Data\n", cycle(), routerID, outLCB[i].link);
            LCBxfer_start (i);
        }
...
//#if 0
            ...
            int vc_selected = 0;
            int curr_vc;
            m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d is ready\n", cycle(), routerID, i);
            ...
            m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d max_count %d\n", cycle(), routerID, i, max_count);
            while (!vc_selected && max_count > 0) {
                if (max_count > ROUTER_NUM_VCS) {
                    ...
                    if ( oLCB->vcTokens[curr_vc] < MaxPacketSize) {
                        ...
                        if (oLCB->ready_outQ_count[curr_vc]<=0)
                            printf ("%" PRId64 ": router %d oLCB %d skipped VC %d skipped with %d tokens and %d ready\n",
                                    cycle(), routerID, i, curr_vc, oLCB->vcTokens[curr_vc],
                                    oLCB->ready_outQ_count[curr_vc]);

                        m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d skipped VC %d skipped with %d tokens and %d ready\n",
                                cycle(), routerID, i, curr_vc, oLCB->vcTokens[curr_vc], oLCB->ready_outQ_count[curr_vc]);
                    } else {
                        m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d skipped VC %d selected\n",
                                  cycle(), routerID, i, curr_vc);
                        if (oLCB->ready_outQ_count[curr_vc]<=0)
                            printf ("%" PRId64 ": router %d oLCB %d skipped VC %d selected with %d tokens and %d ready\n",
                                    cycle(), routerID, i, curr_vc, oLCB->vcTokens[curr_vc],
                                    oLCB->ready_outQ_count[curr_vc]);
                    ...
                    if ((oLCB->vcTokens[curr_vc] < MaxPacketSize)
                        ...
                        if (!(in_queue & (1 << curr_vc))) {
                            ...
                            in_queue |= 1 << curr_vc;
                            if (oLCB->ready_outQ_count[curr_vc]<=0)
                                printf ("%" PRId64 ": router %d oLCB %d regular VC %d skipped with %d tokens and %d ready\n",
                                        cycle(), routerID, i, curr_vc, oLCB->vcTokens[curr_vc],
                                        oLCB->ready_outQ_count[curr_vc]);

                            m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d regular VC %d skipped with %d tokens and %d ready\n",
                                      cycle(), routerID, i, curr_vc, oLCB->vcTokens[curr_vc],
                                      oLCB->ready_outQ_count[curr_vc]);
                        }
                    } else if (oLCB->ready_outQ_count[curr_vc] > 0) {
                        m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d regular VC %d selected\n",
                                  cycle(), routerID, i, curr_vc);
                        if (in_queue & (1 << curr_vc))
                            printf("%" PRId64 ": router %d oLCB %d selecting VC %d in queue\n",
                                   cycle(), routerID, i, curr_vc);
                ...
                max_count--;
                m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d max_count decremented to %d\n", cycle(), routerID, i, max_count);
            }

            if (vc_selected == 0) {
                m_dbg.output(CALL_INFO, "%" PRId64 ": router %d oLCB %d no VCs ready\n",
                          cycle(), routerID, i);
                continue;
            }
            ...
            m_dbg.output(CALL_INFO, "%ld: router %d has output Q %d:%d:%d ready, size %d\n",
                      (unsigned long) cycle(), routerID, oLCB->link, ilink, ovc, (int)theQ->size());

            rp = theQ->front();
            m_dbg.output(CALL_INFO, "%ld: router %d out Q %d:%d:%d trying to send parcel to oLCB\n", 
                      (unsigned long) cycle(),
