...
MultiThreadL1::MultiThreadL1(ComponentId_t id, Params &params) : Component(id) {
    ...
    /* Setup clock */
    clockHandler = new Clock::Handler<MultiThreadL1, &MultiThreadL1::tick>(this);
    ...
    if (isPortConnected("cache")) {
        cacheLink = configureLink("cache", "50ps", new Event::Handler<MultiThreadL1, &MultiThreadL1::handleResponse>(this));
        if (!cacheLink)
    ...
    while (isPortConnected(linkname)) {
        SST::Link * link = configureLink(linkname, "50ps", new Event::Handler<MultiThreadL1, &MultiThreadL1::handleRequest, unsigned int>(this, linkid));
        if (!link)
