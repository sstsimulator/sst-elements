...
#define MEMLINKBASE_ELI_PARAMS { "debug",              "(int) Where to print debug output. Options: 0[no output], 1[stdout], 2[stderr], 3[file]", "0"},\
    ...
    MemLinkBase(Component * comp, Params &params) : SubComponent(comp) {
        ...
        uint32_t node = params.find<uint32_t>("node", 0);
    ...
    void addSource(EndpointInfo srcInfo) { sourceEndpointInfo.insert(srcInfo); }
    void addDest(EndpointInfo dstInfo) { if (info.node == dstInfo.node || dstInfo.node == 9999) destEndpointInfo.insert(dstInfo); }
...
protected:
    ...
    // only for L2 cache: if shared memory exists forward shared memory requests down to L3 cache
