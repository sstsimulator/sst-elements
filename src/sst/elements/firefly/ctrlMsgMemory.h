...
class Memory : public MemoryBase {
  ...
  public:
    typedef std::function<void(Callback,uint64_t)> SchedCallback;
    
    Memory( Component* comp, Params& params, Output& dbg, SchedCallback delayCB );
    ~Memory();
    ...
    virtual void copy( Callback callback, MemAddr to, MemAddr from, size_t length ) {
        ...
        m_delayCB( callback, delay );
    }

    virtual void write( Callback callback, MemAddr to, size_t length ) {
        m_dbg.verbose(CALL_INFO,1,1,"\n");
        m_delayCB( callback, txMemcpyDelay( length ) );
    } 
    ...
    virtual void pin( Callback callback, MemAddr, size_t length ) {
        m_dbg.verbose(CALL_INFO,1,1,"\n");
        m_delayCB( callback, regRegionDelay( length ) );
    }
    ...
    virtual void walk( Callback callback, int count ) {
        m_dbg.verbose(CALL_INFO,1,1,"\n");
        m_delayCB( callback, matchDelay( count ) );
    }
  ...
  private:
    ...
    Output& m_dbg;
    SchedCallback m_delayCB;
    LatencyMod* m_txMemcpyMod;
    LatencyMod* m_rxMemcpyMod;
    Link*       m_delayLink;
};

inline Memory::Memory( Component* comp, Params& params, Output& dbg, SchedCallback delayCB ) : m_dbg(dbg), m_delayCB( delayCB ) 
{
