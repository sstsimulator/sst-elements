...
{
    clock_handler_ptr = new Clock::Handler<iris_trig_nic>(this, &iris_trig_nic::clock_handler);
    ...
    cpu_link = configureLink( "cpu", new Event::Handler<iris_trig_nic>(this,&iris_trig_nic::processCPUEvent) );
    ...
    // appropriate time.
    ptl_link = configureSelfLink("self_ptl", new Event::Handler<iris_trig_nic>(this,&iris_trig_nic::processPtlEvent) );

    // Set up the dma link
    dma_link = configureSelfLink("self_dma", new Event::Handler<iris_trig_nic>(this,&iris_trig_nic::processDMAEvent) );
...
void iris_trig_nic::processCPUEvent( Event *e) {
...
void
iris_trig_nic::setTimingParams(int set) {
    if ( set == 1 ) {
	ptl_msg_latency = 25;
	ptl_unit_latency = 50;
	latency_ct_post = 25;
...
void iris_trig_nic::processPtlEvent( Event *e ) {
...
void
iris_trig_nic::processDMAEvent( Event* e)
{
...
void
iris_trig_nic::scheduleCTInc(ptl_handle_ct_t ct_handle, ptl_size_t increment, SimTime_t delay) {
...
void
iris_trig_nic::scheduleUpdateHostCT(ptl_handle_ct_t ct_handle) {
    ptl_update_ct_event_t* ct_update = new ptl_update_ct_event_t;
...
void
iris_trig_nic::scheduleEQ(ptl_handle_eq_t eq_handle, ptl_event_t* ptl_event) {
...
double
iris_trig_nic::computeDoubleAtomic(unsigned long addr, double value, ptl_op_t op)
{
...
int64_t
iris_trig_nic::computeIntAtomic(unsigned long addr, int64_t value, ptl_op_t op)
{
    atomic_cache_entry* entry;
    if ( atomic_cache.find(addr) != atomic_cache.end() ) {
	entry = atomic_cache[addr];
...
ptl_int_me_t*
iris_trig_nic::match_header(me_list_t* list, ptl_header_t* header, ptl_size_t* offset, ptl_size_t* length)
{
    ptl_int_me_t* match_me;
    bool found = false;
