...
#ifndef COMPONENTS_FIREFLY_CTRLMSGREGREGIONSTATE_H
#define COMPONENTS_FIREFLY_CTRLMSGREGREGIONSTATE_H
...
template< class T1 >
class RegRegionState : StateBase< T1 > 
{
  public:
    RegRegionState( int verbose, Output::output_location_t loc, T1& obj ) : 
        StateBase<T1>( verbose, loc, obj ),
        m_unblock( this, &RegRegionState<T1>::unblock )
    {
        char buffer[100];
        snprintf(buffer,100,"@t:%d:%d:CtrlMsg::RegRegionState::@p():@l ",
  ...
  private:
    ...
    Functor_0<RegRegionState<T1>,bool> m_unblock;
    FunctorBase_0<bool>*    m_functor;
};

template< class T1 >
void RegRegionState<T1>::enter( nid_t nid, region_t region , void* buf, size_t len,
             FunctorBase_0<bool>* functor, FunctorBase_0<bool>* stateFunctor)
{
    ...
    obj().m_processQueuesState->enterRegRegion( nid, region, buf, len, &m_unblock ); 
}

template< class T1 >
bool RegRegionState<T1>::unblock()
{
