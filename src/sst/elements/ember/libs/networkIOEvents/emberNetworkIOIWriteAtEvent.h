// Copyright 2013-2026 NTESS. Under the terms
// of Contract DE-NA0003525 with NTESS, the U.S.
// Government retains certain rights in this software.
//
// Copyright (c) 2013-2026, NTESS
// All rights reserved.
//
// Portions are copyright of other developers:
// See the file CONTRIBUTORS.TXT in the top level directory
// of the distribution for more information.
//
// This file is part of the SST software package. For license
// information, see the LICENSE file in the top level directory of the
// distribution.

#pragma once

#include "emberNetworkIOEvent.h"
#include <cstdlib>

namespace SST {
namespace Ember {

class EmberNetworkIOIWriteAtEvent : public EmberNetworkIOEvent {
public:
    EmberNetworkIOIWriteAtEvent(NetworkIO::Interface& api, Output* output,
                                 NetworkIO::FileHandle handle,
                                 uint64_t offset, Hermes::MemAddr src,
                                 uint64_t length,
                                 NetworkIO::IORequest* outReq,
                                 EmberEventTimeStatistic* stat = nullptr) :
        EmberNetworkIOEvent(api, output, stat),
        m_handle(handle), m_offset(offset), m_src(src),
        m_length(length), m_outReq(outReq)
    {}

    ~EmberNetworkIOIWriteAtEvent() {}

    std::string getName() { return "NetworkIOIWriteAt"; }

    virtual void issue(uint64_t time, Callback callback) {
        // PR 10: non-blocking. See EmberNetworkIOIReadAtEvent.h for the
        // rationale (mirror of MPI_File_iwrite_at returning immediately).
        EmberEvent::issue(time);
        m_api.iwrite_at(m_handle, m_offset, m_src.getSimVAddr(),
                        m_length, m_outReq, NetworkIO::StatusCallback());
        callback(0);
    }

    virtual bool complete(uint64_t time, int retval = 0) {
        return EmberEvent::complete(time, retval);
    }

private:
    NetworkIO::FileHandle m_handle;
    uint64_t m_offset;
    Hermes::MemAddr m_src;
    uint64_t m_length;
    NetworkIO::IORequest* m_outReq;
};

}
}
