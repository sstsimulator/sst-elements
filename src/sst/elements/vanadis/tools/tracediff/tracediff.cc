...
void
read_line(FILE* input_file, char* buffer) {
    buffer[0] = '\0';
    int next_index = 0;
...
void
generate_error(int left_line, int right_line, const char* error_msg) {
    ...
    while (keep_processing) {
        if (feof(left_file)) {
            if (feof(right_file)) {
                ...
                generate_error(left_line_index, right_line_index,
                               "Left file end but right file end not reached, right "
                               "file is longer.");
                keep_processing = false;
            }
        } else {
            if (feof(right_file)) {
                generate_error(left_line_index, right_line_index,
                               "Right file end reached, but left file end not reached, "
                               "left file is longer.");
                keep_processing = false;
            } else {
