...
# SST_SWEEP_openMP_LIST=first-last  -- Sweep Tests to be run
#
################################################################################
...
def build_sweep_test_matrix():
    ...
    # L1 Cache Options
    L1_cache_size_list = ["8KB", "64KB"]
    L1_associativity_list = ["2", "4", "8"]

    # L2 Cache Options
    L2_cache_size_list = ["32KB", "512KB", "1MB"]
    L2_associativity_list = ["8", "16"]
    L2_mshr_size_list = ["8", "64"]

    # Both Cache Replacment Policy
    replacement_policy_list = ["lru", "lfu", "random", "mru"]
...
####
    ...
    def test_memH_sweep_openMP(self, testnum, omp_app, L1_size, L1_asso, L2_size, L2_asso, L2_mshr, repl, test_type, prefetch):
        self._checkSkipConditions(testnum)

        log_debug("Running memH sweep openMP #{0:04}; App={1}; L1:size={2}; asso={3}; L2:size={4}; asso={5}; mshr={6}; cache_repl={7}; test_type={8}; prefetch={9}".format(testnum, omp_app, L1_size, L1_asso, L2_size, L2_asso, L2_mshr, repl, test_type, prefetch))
...
####
    ...
    def memH_sweep_test_template(self, testnum, omp_app, L1_size, L1_asso, L2_size, L2_asso, L2_mshr, repl, test_type, prefetch):
        ...
        testDataFileName="test_memH_sweep_openMP_{0}_case".format(omp_app)
        ...
        sdlfile = "{0}/openMP/sweepopenmp.py".format(test_path)
        testtimeout = 120

        # Set the environment path to the omp application used by the SDL file
        # and verify it exists
        ompapppath = "{0}/openMP/{1}/{1}".format(test_path, omp_app)
        self.assertTrue(os.path.isfile(ompapppath), "Sweep openMP Test; ompfile {0} - not found".format(ompapppath))
        os.environ["OMP_EXE"] = ompapppath

        # Set the model options for the test (This is what we sweep)
        otherargs = '--model-options="--L1cachesz={0} --L1assoc={1} --L2cachesz={2} --L2assoc={3} --L2MSHR={4} --Replacp={5} --MSIMESI={6} --Pref={7}" '.format(
                    L1_size,
                    L1_asso,
                    L2_size,
                    L2_asso,
        ...
        self.assertTrue(outtestresult, "Sweep openMP Test {0} - Cannot find string \"{1}\" in output file {2}".format(testnum, grepstr, outfile))
...
###############################################
    ...
    def _checkSkipConditions(self, testindex):
        ...
        try:
            ...
            log_debug("SST_SWEEP_openMP_LIST = {0}; type = {1}".format(testlist, type(testlist)))
            index = testlist.find('-')
