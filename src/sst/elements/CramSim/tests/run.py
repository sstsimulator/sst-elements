...
sstCmds = {
	...
	14:"sst --lib-path=../.libs ./test_txntrace.py --model-options=\"--configfile=%s traceFile=../traces/usimm.trc stopAtCycle=%s traceFileType=USIMM\"" % (sysconfig,stopAtCycle),		# basic test with verimem_1_RW trace
	15:"sst --lib-path=../.libs ./test_multilanes_4lane.py --model-options=\"--configfile=../ddr4_2400.cfg stopAtCycle=%s\"" % (stopAtCycle),						# basic test for txn dispatcher with 1lane
