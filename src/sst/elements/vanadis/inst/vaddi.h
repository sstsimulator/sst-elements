...
public:
	...
	VanadisAddImmInstruction* clone() {
		return new VanadisAddImmInstruction( *this );
	}
	...
	virtual void printToBuffer(char* buffer, size_t buffer_size) {
                snprintf(buffer, buffer_size, "ADDI    %5" PRIu16 " <- %5" PRIu16 " + imm=%" PRId64 " (phys: %5" PRIu16 " <- %5" PRIu16 " + %" PRId64 ")",
	...
	virtual void execute( SST::Output* output, VanadisRegisterFile* regFile ) {
		...
		output->verbose(CALL_INFO, 16, 0, "Execute: (addr=%p) ADDI phys: out=%" PRIu16 " in=%" PRIu16 " imm=%" PRId64 ", isa: out=%" PRIu16 " / in=%" PRIu16 "\n",
