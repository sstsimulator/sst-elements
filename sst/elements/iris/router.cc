...
Router::do_ib (const irisNPkt* hf, uint16_t p )
{
    ...
    ib_state.at(p*r_param->vcs+v).in_channel = hf->vc;
    ib_state.at(p*r_param->vcs+v).packet_length = hf->sizeInFlits;
    ib_state.at(p*r_param->vcs+v).pipe_stage = IB;
    ib_state.at(p*r_param->vcs+v).mclass = (message_class_t)hf->mclass;
    ...
    ib_state.at(p*r_param->vcs+v).src_node_id = hf->srcNum;
    ib_state.at(p*r_param->vcs+v).dst_node_id = hf->destNum;
