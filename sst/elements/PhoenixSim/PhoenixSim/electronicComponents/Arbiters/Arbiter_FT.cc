...
bool Arbiter_FT::seeded = false;

Arbiter_FT::Arbiter_FT() {
	if(!seeded){
		srand(time(NULL));
		seeded = true;
	}
...
bool Arbiter_FT::isDimensionChange(int in, int out) {
	return false;
}

//returns the port num to go to
int Arbiter_FT::route(ArbiterRequestMsg* rmsg) {

	return getUpPort(rmsg, level);

}

int Arbiter_FT::getUpPort(ArbiterRequestMsg* rmsg, int lev) {
	debug(name, "calling upPort.. ", UNIT_ROUTER);

	return (intrand(2) >= 1) ? FT_UP1 : FT_UP0;
}

int Arbiter_FT::getDownPort(ArbiterRequestMsg* rmsg, int lev) {
	debug(name, "calling downPort.. ", UNIT_ROUTER);

	NetworkAddress* addr = (NetworkAddress*)rmsg->getDest();
	int destId = addr->id[lev];

	return (destId == 0) ? FT_DN0 : FT_DN1;
}

void Arbiter_FT::PSEsetup(int inport, int outport, PSE_STATE st) {
