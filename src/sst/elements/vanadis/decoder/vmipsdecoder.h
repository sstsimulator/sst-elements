...
public:
    ...
    virtual void configureApplicationLaunch(SST::Output* output, VanadisISATable* isa_tbl, VanadisRegisterFile* regFile,
                                            VanadisLoadStoreQueue* lsq, VanadisELFInfo* elf_info, SST::Params& params) {
	...
	// Check endian-ness
	if( elf_info->getEndian() != LITTLE_ENDIAN ) {
        ...
        output->verbose(CALL_INFO, 16, 0,
                        "Aligning stack address to 64 bytes (%" PRIu64 " - %" PRIu64 " - padding: %" PRIu64
                        " = %" PRIu64 " / 0x%0llx)\n",
                        start_stack_address, (uint64_t)arg_env_space_and_data_needed, padding_needed,
        ...
        for (size_t i = 0; i < arg_start_offsets.size(); ++i) {
            output->verbose(CALL_INFO, 16, 0, "--> Setting arg%" PRIu32 " to point to address %" PRIu64 " / 0x%llx\n",
                            (uint32_t)i, arg_env_data_start + arg_start_offsets[i],
                            arg_env_data_start + arg_start_offsets[i]);
        ...
        output->verbose(CALL_INFO, 16, 0,
                        "-> Pushing %" PRIu64 " bytes to the start of stack (0x%llx) via memory init event..\n",
    ...
    virtual void tick(SST::Output* output, uint64_t cycle) {
                    ...
                    if (bundle->getInstructionByIndex(bundle->getInstructionCount() - 1)->getInstFuncType()
                        ...
                        if (ins_loader->hasBundleAt(ip + 4)) {
                            ...
                            if (ins_loader->hasPredecodeAt(ip + 4)) {
                                ...
                                if (ins_loader->getPredecodeBytes(output, ip + 4, (uint8_t*)&temp_delay,
                                                                  sizeof(temp_delay))) {
                        ...
                        if (bundle->getInstructionCount() < (thread_rob->capacity() - thread_rob->size())) {
                            ...
                            output->verbose(CALL_INFO, 16, 0,
                                            "---> --> micro-op bundle for %p contains %" PRIu32
                                            " ops, we only have %" PRIu32
        ...
        output->verbose(CALL_INFO, 16, 0,
                        "---> Performed %" PRIu16 " decodes this cycle, %" PRIu16
                        " uop-bundles used / updated-ip: 0x%llx.\n",
...
protected:
    ...
    void decode(SST::Output* output, const uint64_t ins_addr, const uint32_t next_ins,
        ...
        if (0 == next_ins) {
            ...
            case MIPS_SPEC_OP_MASK_COP1: {
                ...
                if ((next_ins & 0x3E30000) == 0x1010000) {
                    ...
                    bundle->addInstruction(
                        new VanadisBranchFPInstruction(ins_addr, hw_thr, options, MIPS_FP_STATUS_REG, imm_value_64,
        ...
        for (uint32_t i = 0; i < bundle->getInstructionCount(); ++i) {
            output->verbose(CALL_INFO, 16, 0, "-> [%3" PRIu32 "]: %s\n", i,
                            bundle->getInstructionByIndex(i)->getInstCode());
        }
