/* alloc-atom.cpp - Detailed Pipeline Model */
/*
 * __COPYRIGHT__ GT
 */
  ...
  if(!strcasecmp(alloc_opt_string,"ATOM"))
    return new core_alloc_atom_t(core);
#else

class core_alloc_atom_t:public core_alloc_t
{
  ...
  core_alloc_atom_t(struct core_t * const core);
...
const char *core_alloc_atom_t::alloc_stall_str[ASTALL_num] = {
  "no stall                   ",
  "no uops to allocate        ",
  "RS is full                 ",
  "OOO core draining          "
...
core_alloc_atom_t::core_alloc_atom_t(struct core_t * const arg_core)
{
...
void
core_alloc_atom_t::reg_stats(struct stat_sdb_t * const sdb)
{
  char buf[1024];
  char buf2[1024];
...
void core_alloc_atom_t::step(void)
{
  ...
  /*========================================================================*/
  /*== Dispatch insts if RS entries available (as needed) ==*/
    ...
    for(i=0; i < knobs->alloc.width; i++) /* if so, scan all slots (width) of this stage */
    {
      ...
      if(uop)
      {
		...
		/* For inorder cores there is no separate functional unit for load/stores. the adder functional 
                   units handle the load/store pipelines and all uops go through the load/store pipelines. */
		uop->decode.FU_class = REG_IS_FPR(uop->decode.odep_name)?FU_FADD:FU_IEU;

		/* Loads do not have a separate execution unit*/
		int port;
                if(knobs->exec.port_binding[uop->decode.FU_class].num_FUs == 1)
                {
			port=REG_IS_FPR(uop->decode.odep_name)?2:knobs->exec.port_binding[uop->decode.FU_class].ports[0];
		}else//More than one functional units available need to find the least loaded port
		{
			int min_load = INT_MAX;
                	int index = -1;
                	for(int j=0;j<knobs->exec.port_binding[uop->decode.FU_class].num_FUs;j++)
                	{
                  		int port2 = knobs->exec.port_binding[uop->decode.FU_class].ports[j];
                  		if(port_loading[port2] < min_load)
                  		{
                    			min_load = port_loading[port2];
                    			index = port2;
                  		}
                	}
			port=index;
		}
		if(!uop->decode.is_nop && (uop->decode.is_fusion_head || (!uop->decode.in_fusion)) )
	        {
#ifdef ZDEBUG
    if(core->sim_cycle > PRINT_CYCLE )
	fprintf(stdout,"\n[%lld][Core%d]ALLOC MOP=%d UOP=%d in Looking for RS entry(Memory Instruction) %d in_fusion %d port %d uop->FU Class %d",core->sim_cycle,core->id,uop->decode.Mop_seq,uop->decode.uop_seq,port,uop->decode.in_fusion,port,uop->decode.FU_class);
#endif
		      if(!core->exec->RS_available(port))
		      {
		...
		int port;
                if(knobs->exec.port_binding[uop->decode.FU_class].num_FUs == 1)
                {
			port=knobs->exec.port_binding[uop->decode.FU_class].ports[0];
		}else//More than one functional units available need to find the least loaded port
		{
			int min_load = INT_MAX;
                	int index = -1;
		...
		if(!uop->decode.is_nop && (!uop->decode.in_fusion || uop->decode.is_fusion_head) )
	        {
#ifdef ZDEBUG
    if(core->sim_cycle > PRINT_CYCLE )
	fprintf(stdout,"\n[%lld][Core%d]ALLOC MOP=%d UOP=%d in Looking for RS entry(Non Memory instruction) %d in_fusion %d port %d uop->FU Class %d",core->sim_cycle,core->id,uop->decode.Mop_seq,uop->decode.uop_seq,knobs->exec.port_binding[uop->decode.FU_class].ports[0],uop->decode.in_fusion,port,uop->decode.FU_class);
#endif
		      if(!core->exec->RS_available(port))
        	      {
			stall_reason = ASTALL_RS;
            ...
            /* place in ROB, ROB is just a simulation structure here to keep track of the entries rather than a microarchitectural entity */
		...
		if(uop->decode.is_load || uop->decode.is_sta || uop->decode.is_std)
		{	
			if( REG_IS_FPR(uop->decode.odep_name) )
				uop->alloc.port_assignment = 2;
			else
		...
		if( (uop->decode.is_load || uop->decode.is_sta || uop->decode.is_std) && REG_IS_FPR(uop->decode.odep_name))
			uop->alloc.port_assignment = 2;
		if(uop->decode.in_fusion && !uop->decode.is_fusion_head)
			uop->alloc.port_assignment = uop->decode.fusion_head->alloc.port_assignment;			
              }
...
#ifdef ZDEBUG
    if(core->sim_cycle > PRINT_CYCLE )
	fprintf(stdout,"\n[%lld][Core%d]ALLOC MOP=%d UOP=%d in RS %d in_fusion %d",core->sim_cycle,core->id,uop->decode.Mop_seq,uop->decode.uop_seq,uop->alloc.port_assignment,uop->decode.in_fusion);
#endif
...
#ifdef ZDEBUG
    if(core->sim_cycle > PRINT_CYCLE )
	fprintf(stdout,"\n[%lld][Core%d]ALLOC MOP=%d UOP=%d in RS(FUSED) %d in_fusion %d head_uops RS %d",core->sim_cycle,core->id,uop->decode.Mop_seq,uop->decode.uop_seq,uop->alloc.port_assignment,uop->decode.in_fusion,uop->decode.fusion_head->alloc.port_assignment);
#endif
	  ...
	  {
	    //fprintf(stdout,"  Going to next uop " );
        ...
        if((!pipe[stage][i]->decode.in_fusion) || !uop) /* either not fused , or complete fused uops alloc'd - Only Mops which were fused but not completely alocated */
        {
          uop = pipe[stage][i]; /* may be NULL if we just finished a fused set, thus we need to reassign */
...
/* start from most recently fetched, blow away everything until
   we find the Mop */
void
core_alloc_atom_t::recover(const struct Mop_t * const Mop)
{
  struct core_knobs_t * knobs = core->knobs;
  int stage,i;
...
void
core_alloc_atom_t::recover(void)
{
  struct core_knobs_t * knobs = core->knobs;
  int stage,i;
  for(stage=0;stage<knobs->alloc.depth;stage++)
...
void core_alloc_atom_t::RS_deallocate(const struct uop_t * const uop)
{
#ifdef ZDEBUG
    if(core->sim_cycle > PRINT_CYCLE )
	fprintf(stdout,"\n[%lld][Core%d]ALLOC MOP=%d UOP=%d in RS deallocate %d in_fusion %d",core->sim_cycle,core->id,uop->decode.Mop_seq,uop->decode.uop_seq,uop->alloc.port_assignment,uop->decode.in_fusion);
#endif
  zesto_assert(port_loading[uop->alloc.port_assignment] >= 0,(void)0);
  port_loading[uop->alloc.port_assignment]--;
}

void core_alloc_atom_t::start_drain(void)
{
