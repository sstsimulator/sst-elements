...
EmberSweep3DGenerator::EmberSweep3DGenerator(SST::Component* owner, Params& params) :
	...
	px = (uint32_t) params.find_integer("px", 0);
	py = (uint32_t) params.find_integer("py", 0);
...
void EmberSweep3DGenerator::configureEnvironment(const SST::Output* output, uint32_t pRank, uint32_t worldSize) {
	...
	x_up   = (myX != (px - 1)) ? myX + 1 : -1;
	x_down = (myX != 0) ? myX - 1 : -1;

	y_up   = (myY != (py - 1)) ? myY + 1 : -1;
	y_down = (myY != 0) ? myY - 1 : -1;
...
void EmberSweep3DGenerator::generate(const SST::Output* output, const uint32_t phase, std::queue<EmberEvent*>* evQ) {
	if(phase < iterations) {
		...
		for(uint32_t i = 0; i < nz; i+= kba) {
			if(x_down > 0) {
				evQ->push( new EmberRecvEvent(x_down, (nx * kba), 0, (Communicator) 0) );
			}

			if(y_down > 0) {
				evQ->push( new EmberRecvEvent(y_down, (ny * kba), 0, (Communicator) 0) );
			}

			evQ->push(new EmberComputeEvent(nsCompute));

			if(x_up > 0) {
                                evQ->push( new EmberSendEvent(x_up, (nx * kba), 0, (Communicator) 0) );
                        }

                        if(y_up > 0) {
                                evQ->push( new EmberRecvEvent(y_up, (ny * kba), 0, (Communicator) 0) );
                        }
		}
		...
		for(uint32_t i = 0; i < nz; i+= kba) {
			if(x_up > 0) {
				evQ->push( new EmberRecvEvent(x_up, (nx * kba), 0, (Communicator) 0) );
			}
			...
			if(x_down > 0) {
                                evQ->push( new EmberSendEvent(x_down, (nx * kba), 0, (Communicator) 0) );
                        }
