...
MemNICMulti::MemNICMulti(Component * parent, Params &params) : MemNICBase(parent, params) {
    ...
    bool found;
    std::array<std::string,2> pref = {"req", "data"};
    ...
    for (int i = 0; i < 2; i++) {
        ...
        link_control[i] = (SimpleNetwork*)parent->loadSubComponent("shogun.ShogunNIC", parent, params);
    ...
    link_control[REQ]->setNotifyOnReceive(new SimpleNetwork::Handler<MemNICMulti>(this, &MemNICMulti::recvNotifyReq));
    link_control[DATA]->setNotifyOnReceive(new SimpleNetwork::Handler<MemNICMulti>(this, &MemNICMulti::recvNotifyData));
//     link_control[ACK]->setNotifyOnReceive(new SimpleNetwork::Handler<MemNICMulti>(this, &MemNICMulti::recvNotifyAck));
//     link_control[FWD]->setNotifyOnReceive(new SimpleNetwork::Handler<MemNICMulti>(this, &MemNICMulti::recvNotifyFwd));
...
/* Debug info - record before we attempt send so that if send destroys anything we have it */
            ...
            if (link_control[j]->spaceToSend(0, head->size_in_bits) && link_control[j]->send(head, 0)) {
#ifdef __SST_DEBUG_OUTPUT__
 ...
 */
bool MemNICMulti::recvNotifyReq(int) {
...
void MemNICMulti::doRecv(SimpleNetwork::Request * req, NetType net) {
...
void MemNICMulti::recvNotify(OrderedMemRtrEvent* mre) {
...
MemNICMulti::OrderedMemRtrEvent* MemNICMulti::processRecv(SimpleNetwork::Request * req) {
    if (req != nullptr) {
...
size_t MemNICMulti::getSizeInBits(MemEventBase *ev, NetType net) {
    return 8 * (packetHeaderBytes[net] + ev->getPayloadSize());
}

// If statements catch special case of fatal() during construction...
void MemNICMulti::printStatus(Output& out) {
    out.output("    Begin MemHierarchy::MemNICMulti %s\n", getName().c_str());
    ...
    out.output("    End MemHierarchy::MemNICMulti\n");
}
