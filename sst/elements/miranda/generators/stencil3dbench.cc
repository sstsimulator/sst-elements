...
void Stencil3DBenchGenerator::generate(std::queue<RequestGeneratorRequest*>* q) {
	...
	for(uint32_t curZ = (startZ + 1); curZ < (endZ - 1); curZ++) {
		for(uint32_t curY = 1; curY < (nY - 1); curY++) {
			for(uint32_t curX = 1; curX < (nX - 1); curX++) {
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX - 1, curY - 1, curZ - 1), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX,     curY - 1, curZ - 1), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX + 1, curY - 1, curZ - 1), datawidth, READ));

				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX - 1, curY    , curZ - 1), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX,     curY    , curZ - 1), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX + 1, curY    , curZ - 1), datawidth, READ));
				...
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX - 1, curY - 1, curZ    ), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX,     curY - 1, curZ    ), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX + 1, curY - 1, curZ    ), datawidth, READ));

				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX - 1, curY    , curZ    ), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX,     curY    , curZ    ), datawidth, READ));
				q->push(new RequestGeneratorRequest(datawidth * convertPositionToIndex(curX + 1, curY    , curZ    ), datawidth, READ));
				...
				q->push(new RequestGeneratorRequest( (nX * nY * nZ * datawidth) +
