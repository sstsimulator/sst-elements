...
template <typename fp_format, typename gpr_format, bool is_bitwise>
class VanadisSIMTFP2GPRInstruction : public VanadisSIMTInstruction, public VanadisFP2GPRInstruction<fp_format, gpr_format, is_bitwise>
{
public:
    VanadisSIMTFP2GPRInstruction(const uint64_t addr, const uint32_t hw_thr, const VanadisDecoderOptions* isa_opts, 
    VanadisFloatingPointFlags* fpflags, const uint16_t int_dest, const uint16_t fp_src) :
         VanadisFP2GPRInstruction<fp_format, gpr_format, is_bitwise>(addr, hw_thr, isa_opts, fpflags, int_dest, fp_src),
        VanadisFloatingPointInstruction(addr, hw_thr, isa_opts, fpflags, 0, 1, 0, 1,
            ((sizeof(fp_format) == 8) && (VANADIS_REGISTER_MODE_FP32 == isa_opts->getFPRegisterMode())) ? 2 : 1, 0,
            ((sizeof(fp_format) == 8) && (VANADIS_REGISTER_MODE_FP32 == isa_opts->getFPRegisterMode())) ? 2 : 1, 0),
        VanadisInstruction(addr, hw_thr, isa_opts, 0, 1, 0, 1,
            ((sizeof(fp_format) == 8) && (VANADIS_REGISTER_MODE_FP32 == isa_opts->getFPRegisterMode())) ? 2 : 1, 0,
            ((sizeof(fp_format) == 8) && (VANADIS_REGISTER_MODE_FP32 == isa_opts->getFPRegisterMode())) ? 2 : 1, 0),
        VanadisSIMTInstruction(addr, hw_thr, isa_opts, 0, 1, 0, 1,
            ((sizeof(fp_format) == 8) && (VANADIS_REGISTER_MODE_FP32 == isa_opts->getFPRegisterMode())) ? 2 : 1, 0,
            ((sizeof(fp_format) == 8) && (VANADIS_REGISTER_MODE_FP32 == isa_opts->getFPRegisterMode())) ? 2 : 1, 0)  
    {
    ...
    VanadisSIMTFP2GPRInstruction* clone() override { return new VanadisSIMTFP2GPRInstruction(*this); }
    ...
    void simtExecute(SST::Output* output, VanadisRegisterFile* regFile) override
    {
        ...
        VanadisFP2GPRInstruction<fp_format, gpr_format, is_bitwise>::instOp(regFile,phys_fp_regs_in_0,phys_fp_regs_in_1,phys_int_regs_out_0 );
        VanadisFP2GPRInstruction<fp_format, gpr_format, is_bitwise>::log(output, 16, VanadisSIMTInstruction::sw_thread,phys_int_regs_out_0,phys_fp_regs_in_0);
    }
