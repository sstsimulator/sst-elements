...
using namespace McNiagara;
	...
	//[NUM_INSTPROBS+1]
	/*
	   vector<string> McNiagara::instructionProbNames = {
	   ...
	   "D_FLOATS", "D_INTS"};//, 0 };
//[NUM_PERFCTRS+1]
vector<string> McNiagara::performanceCtrNames = {
...
{
	test_var=2;
	cout<<"McNiagara: Constructor"<<endl;
	instructionProbNames.push_back("PB_6_CTI_N");
	instructionProbNames.push_back("PB_6_INT_N");
	instructionProbNames.push_back("PB_25_INT_N");
	instructionProbNames.push_back("PB_6_FGU_N");
	instructionProbNames.push_back("PB_30_FGU_N");
	instructionProbNames.push_back("PB_6_MEM_N");
	instructionProbNames.push_back("PB_25_MEM_N");
	instructionProbNames.push_back("PB_3_LD_N");
	instructionProbNames.push_back("P_FDIV_FSQRT_S_N");
	instructionProbNames.push_back("P_FDIV_FSQRT_D_N");
	instructionProbNames.push_back("PB_6_INT_D_N");
	instructionProbNames.push_back("PB_25_INT_D_N");
	instructionProbNames.push_back("PB_6_FGU_D_N");
	instructionProbNames.push_back("PB_30_FGU_D_N");
	instructionProbNames.push_back("PB_6_MEM_D_N");
	instructionProbNames.push_back("PB_25_MEM_D_N");
	instructionProbNames.push_back("PB_3_LD_D_N");
	instructionProbNames.push_back("P_FDIV_FSQRT_S_D_N");
	instructionProbNames.push_back("P_FDIV_FSQRT_D_D_N");
	instructionProbNames.push_back("P_FDIV_FSQRT_S");
	instructionProbNames.push_back("P_FDIV_FSQRT_D");
	instructionProbNames.push_back("P_DS");
	instructionProbNames.push_back("DELAY_SLOT_N");
	instructionProbNames.push_back("ANNULLED_N");
	instructionProbNames.push_back("D_LOADS");
	instructionProbNames.push_back("D_STORES");
	instructionProbNames.push_back("D_FLOATS");
	instructionProbNames.push_back("D_INTS");

	performanceCtrNames.push_back("TOTAL_CYCLES");
	performanceCtrNames.push_back("L2_MISSES");
	performanceCtrNames.push_back("L2_I_MISSES");
	performanceCtrNames.push_back("L1_MISSES");
	performanceCtrNames.push_back("IC_MISSES");
	performanceCtrNames.push_back("TLB_MISSES");
	performanceCtrNames.push_back("ITLB_MISSES");
	performanceCtrNames.push_back("TAKEN_BRS");
	performanceCtrNames.push_back("TOTAL_INSTS");
	performanceCtrNames.push_back("MEASURED_CPI");
	performanceCtrNames.push_back("TOTAL_LDS");
	performanceCtrNames.push_back("TOTAL_STS");
	performanceCtrNames.push_back("TOTAL_FPS");
	performanceCtrNames.push_back("TOTAL_BRS");
	performanceCtrNames.push_back("LD_PERC");
	performanceCtrNames.push_back("ST_PERC");
	performanceCtrNames.push_back("BR_PERC");
	performanceCtrNames.push_back("FP_PERC");
	performanceCtrNames.push_back("GR_PERC");
...
//                              char *paramnames[])
int McNiagara::read_paramfile(string filename,
		McNiagara::ModelParam params[],
		vector<string> paramnames)
{
	if(Debug>2) cout<<"read_paramfile("<<filename<<")"<<endl;
	unsigned int i;
	ifstream f;//FILE *f;
	string line;//char line[256];
	unsigned long long lv;
	double dv;
	string name,value,temp;//char name[40], value[40];
	enum { NONE, ULL, DBL } valtype;
	f.open(filename.c_str());//f = fopen(filename.c_str(),"r");
	if (!f.is_open()) {
		cerr<<"Error: Cannot open parameter file: '"<<filename<<"'"<<endl;
		return 1;
	}
	while (std::getline(f,line)) //while (fgets(line,sizeof(line),f)) 
	{
	if(line.size()){
		valtype = NONE;
		lv = 0;
		dv = 0;
		//name[39] = value[39] = '\0';
		temp=StringUtil::stringToken(&line,"\t");if(Debug>5) cout<<"temp='"<<temp<<"'"<<endl;
		name=StringUtil::stringToken(0,"\t");if(Debug>5) cout<<"name='"<<name<<"'"<<endl;
		value=StringUtil::stringToken(0,"\t");if(Debug>5) cout<<"value='"<<value<<"'"<<endl;
		if(name.size()&&value.size())//if (sscanf(line,"#define %39s %39s", (char*) &name, (char*) &value) == 2) 
		{
			if(Debug>4) cout<<"   matched ("<<name<<") ("<<value<<")"<<endl;//printf("matched (%s) (%s)\n",name,value);
			if(value.find("L")!=value.npos)//if (strstr(value,"L")) 
			{
				istringstream(value)>>lv;//sscanf(value, "%llu", &lv);
				//printf("ull val = %llu\n",lv);
				valtype = ULL;
			} 
			else if(value.find(".")!=value.npos)//else if (strstr(value,".")) 
			{
				istringstream(value)>>dv;//sscanf(value, "%lg", &dv);
				//printf("dbl val = %lg\n",dv);
				valtype = DBL;
			}
			for(i=0; i<paramnames.size(); ++i) //for (i=0; paramnames[i]; i++)
				if(!paramnames[i].compare(name))//if (!strcmp(paramnames[i], name))
					break;
			if(i<paramnames.size() && valtype != NONE)//if (paramnames[i] && valtype != NONE) 
			{
				params[i].name=paramnames[i];//strcpy(params[i].name, paramnames[i]);
				if (valtype == ULL)
					params[i].v.lval = lv;
				else if (valtype == DBL)
					params[i].v.dval = dv;
				if(Debug>4)cout<<"init "<<params[i].name<<" "<<lv<<" "<<dv<<endl;
			}
		}
	}
	}
...
void McNiagara::init(string in_file, OffCpuIF * extif, 
		string instProbFile, string perfCountFile,
		string tracefile, unsigned long seed)
{
	double p1,
	p2,
	pm;
	...
	if(Debug>0){
		...
		cout<<"Probability File:         '"<<instProbFile<<"'"<<endl;
		cout<<"Performance Counter File: '"<<perfCountFile<<"'"<<endl;
	...
	if(Debug>1) cout<<"<<<<<<<<<<<<Reading Paramter Files>>>>>>>>>>>>>>"<<endl;
	...
	{
		if(Debug>1) cout<<"<<<<<<<<<<<<<<<<Openint Tracefile>>>>>>>>>>>>>>>>"<<endl;
		tracef = fopen(tracefile.c_str(),"r");
		if (!tracef) 
			cerr<<"Error opening trace file ("<<tracefile<<"); running MC..."<<endl;
		//fprintf(stderr,"Error opening trace file (%s); running MC...\n", tracefile);
	}
	if(Debug>1) cout<<">>>>>>>>>>>>>>>>>Done Initializing<<<<<<<<<<<<<<<<<"<<endl;
}
...
int McNiagara::fini(string outfile)
{
	FILE *outf;
	...
	for (r=0; r < CycleTracker::NUMCYCLEREASONS; r++) {
		fprintf(outf, "%13.13s = %4.5f  (%4.02f%% of CPI)\n",
				cycleTracker.categoryName((CycleTracker::CycleReason)r).c_str(),
