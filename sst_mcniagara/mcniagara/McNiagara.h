...
#define TLB_LATENCY  190
	...
	class McNiagara {
		...
		struct ModelParam {
			string name;//char name[32];  ///< parameter string name
			union {
		...
		// static string instructionProbNames[NUM_INSTPROBS+1];
		vector<string> instructionProbNames;
		...
		//static string performanceCtrNames[NUM_PERFCTRS+1]
		vector<string> performanceCtrNames;
		...
		void init(string in_file, OffCpuIF * extif, string instProbFile, 
				string perfCountFile, string tracefile,
				unsigned long seed=0);
		void un_init(void);
		int sim_cycle(unsigned long current_cycle);
		int generate_instruction(Token * token);
		int sim_instruction(Token * token);
		int fini(string outfile);
		//double my_rand(void);
		...
		//      double *fdiv_allowed);
		int read_paramfile(string filename, ModelParam params[],
