...
from HPC_topos import ddf_configs
...
if __name__ == "__main__":
    ...
    # Create Dally Dragonfly topology
    ddf_topo = DallyDragonflyTopo(*ddf_configs[0])  # Use first config: (36, 5)
    ddf_topo.set_endpoints_per_router(2)
    G = ddf_topo.get_nx_graph()

    ### Setup the topology
    topo = topoAny()
    topo.routing_mode = "source_routing"
    topo.topo_name = "dallydragonfly"
    topo.import_graph(G)
