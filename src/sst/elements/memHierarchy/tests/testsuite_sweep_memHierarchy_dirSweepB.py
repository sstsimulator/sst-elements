...
# SST_SWEEP_dirsweepB_LIST=first-last  -- Sweep Tests to be run
#
################################################################################
...
def build_sweep_test_matrix():
    ...
    L1_associativity_list = ["2"]

    # L2 Cache Options
    L2_cache_size_list = ["32KB", "512KB"]
    L2_associativity_list = ["16"]
    L2_mshr_size_list = ["16"]

    # Both Cache Replacment Policy
    replacement_policy_list = ["lru"]
...
####
    ...
    def test_memH_sweep_dirsweepB(self, testnum, omp_app, L1_size, L1_asso, L2_size, L2_asso, L2_mshr, repl, test_type, prefetch):
        self._checkSkipConditions(testnum)

        log_debug("Running memH sweep dirsweepB #{0:04}; App={1}; L1:size={2}; asso={3}; L2:size={4}; asso={5}; mshr={6}; cache_repl={7}; test_type={8}; prefetch={9}".format(testnum, omp_app, L1_size, L1_asso, L2_size, L2_asso, L2_mshr, repl, test_type, prefetch))
...
####
    ...
    def memH_sweep_test_template(self, testnum, omp_app, L1_size, L1_asso, L2_size, L2_asso, L2_mshr, repl, test_type, prefetch):
        ...
        testDataFileName="test_memH_sweep_dirSweepB_{0}_case".format(omp_app)
        ...
        self.assertTrue(os.path.isfile(ompapppath), "Sweep dirsweepB Test; ompfile {0} - not found".format(ompapppath))
        ...
        self.assertTrue(outtestresult, "Sweep dirsweepB Test {0} - Cannot find string \"{1}\" in output file {2}".format(testnum, grepstr, outfile))
...
###############################################
    ...
    def _checkSkipConditions(self, testindex):
        ...
        try:
            ...
            log_debug("SST_SWEEP_dirsweepB_LIST = {0}; type = {1}".format(testlist, type(testlist)))
            index = testlist.find('-')
