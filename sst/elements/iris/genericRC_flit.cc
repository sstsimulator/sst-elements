...
void
GenericRC::route_x_y(const Flit* hf)
{
    ports_t oport = INV;
    uint16_t myx = (int)(node_id%r_param->grid_size);
    uint16_t destx = (int)(hf->pktinfo.destNum%r_param->grid_size);
    uint16_t myy = (int)(node_id/r_param->grid_size);
    uint16_t desty = (int)(hf->pktinfo.destNum/r_param->grid_size);
...
void
GenericRC::route_torus(const Flit* hf)
{
    uint16_t myx = (int)(node_id%r_param->grid_size);
    uint16_t destx = (int)(hf->pktinfo.destNum%r_param->grid_size);
    uint16_t myy = (int)(node_id/r_param->grid_size);
...
void
GenericRC::route_ring_unidirection(const Flit* hf)
{

    r_param->grid_size = r_param->no_nodes;
    uint16_t myx = node_id;
    uint16_t destx = hf->pktinfo.destNum;
    int oport = -1;
    int ovc = -1;
...
void
GenericRC::route_ring(const Flit* hf)
{
...
void
GenericRC::route_twonode(const Flit* hf)
{
    int oport = -1, ovc = -1;

    if ( node_id == 0 )
    {
        if ( hf->pktinfo.destNum== 1)
            oport = 1;
        else
            oport = 0;
    }
