...
from HPC_topos import pf_configs
...
if __name__ == "__main__":
    ...
    # Create Polarfly topology
    pf_topo = PolarflyTopo(*pf_configs[0])  # Use first config: (7, 3)
    pf_topo.set_endpoints_per_router(2)
    G = pf_topo.get_nx_graph()

    ### Setup the topology
    topo = topoAny()
    topo.routing_mode = "source_routing"
    topo.topo_name = "polarfly"
    topo.import_graph(G)
