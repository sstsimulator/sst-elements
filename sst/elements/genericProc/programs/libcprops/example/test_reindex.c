...
void sequence_destroy(sequence *q)
{
	free(q);
}
...
{
	return ((sequence *) q)->id - ((sequence *) s)->id;
}
...
{
	int i;
	for (i = 0; i < sizeof(indices) / sizeof (cp_index *) ; i++)
	{
...
int main(int argc, char *argv[])
{
	...
	/* default index is length. notice only id index is unique. */
	t = cp_multimap_create_by_option(COLLECTION_MODE_MULTIPLE_VALUES | COLLECTION_MODE_COPY | 
									 COLLECTION_MODE_DEEP | 
									 COLLECTION_MODE_NOSYNC,
									 NULL, 
								     (cp_compare_fn) sequence_length_cmp, 
	...
	id_index = 
		cp_multimap_create_index(t, CP_UNIQUE, NULL, sequence_id_cmp, &rc);
	if (id_index == NULL)
	{
		fprintf(stderr, "received error %d\n", rc);
		exit(rc);
	...
	{
		dummy.id = count + 1;
		s = cp_multimap_get_by_index(t, id_index, &dummy);
		if (s == NULL)
		{
