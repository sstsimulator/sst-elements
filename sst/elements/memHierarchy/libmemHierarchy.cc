...
static const ElementInfoParam cache_params[] = {
    ...
    {"replacement_policy",      "Replacement policy of the cache array.  Options:  LRU, LFU, Random, MRU, or NMRU. "},
    ...
    {"directory_at_next_level", "Parameter specifies if there is a flat directory-controller as the next lower level memory: 0, 1; deprecated; set 'bottom_network' to 'directory' instead", "0"},
    {"bottom_network",          "Specifies the type of entity below this cache in the memory hierarchy: network-connected cache ('cache'), directory ('directory') or directly-connected cache ('', default)", ""},
    {"top_network",          "Specifies whether upper level cache is connected directly ('', default) or via a network ('cache')", ""},
    {"num_cache_slices",        "Set this paramater to the total number of distributed/sliced shared caches at this level", "1"},
    {"slice_id",                "Each shared cache at this level should have an ID between 0 and num_cache_slices-1", "0"},
    {"slice_allocation_policy", "Controls how addresses are allocated among distributed shared cache; currently only round-robin distribution, 'rr', implemented", "rr"},
