    ...
    Component(id), rng(id, 13)
{
    ...
    clockHandler = new Clock::Handler<streamCPU, &streamCPU::clockTic>(this);
    clockTC = registerClock(clockFreq, clockHandler);
    num_reads_issued = num_reads_returned = 0;

    memory = loadUserSubComponent<Interfaces::StandardMem>("memory", ComponentInfo::SHARE_NONE, clockTC, new Interfaces::StandardMem::Handler<streamCPU, &streamCPU::handleEvent>(this));
    ...
    if (!memory) {
        ...
        memory = loadAnonymousSubComponent<Interfaces::StandardMem>("memHierarchy.standardInterface", "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
                interfaceParams, clockTC, new Interfaces::StandardMem::Handler<streamCPU, &streamCPU::handleEvent>(this));
