...
c_USimmTraceReader::c_USimmTraceReader(ComponentId_t x_id, Params& x_params) :
	Component(x_id) {
	...
	m_outTxnGenReqPtrLink = configureLink(
			"outTxnGenReqPtr",
			new Event::Handler<c_USimmTraceReader>(this,
					&c_USimmTraceReader::handleOutTxnGenReqPtrEvent));
	//// accept token chg from txn unit
	m_inTxnUnitReqQTokenChgLink = configureLink(
			"inTxnUnitReqQTokenChg",
			new Event::Handler<c_USimmTraceReader>(this,
					&c_USimmTraceReader::handleInTxnUnitReqQTokenChgEvent));
	...
	m_inTxnUnitResPtrLink = configureLink(
			"inTxnUnitResPtr",
			new Event::Handler<c_USimmTraceReader>(this,
					&c_USimmTraceReader::handleInTxnUnitResPtrEvent));
	//// send token chg to txn unit
	m_outTxnGenResQTokenChgLink = configureLink(
			"outTxnGenResQTokenChg",
			new Event::Handler<c_USimmTraceReader>(this,
					&c_USimmTraceReader::handleOutTxnGenResQTokenChgEvent));

	//set our clock
	registerClock("1GHz",
			new Clock::Handler<c_USimmTraceReader>(this, &c_USimmTraceReader::clockTic));
}

c_USimmTraceReader::~c_USimmTraceReader() {
...
bool c_USimmTraceReader::clockTic(Cycle_t) {
...
void c_USimmTraceReader::createTxn() {
  // check if txn can fit inside Req q
    ...
    if (std::getline(*m_traceFileStream, l_line)) {
      ...
      for (Tokenizer<>::iterator l_iter = l_tok.begin();
        ...
        c_Transaction* l_txn = getNextTransaction(l_txnType, l_txnAddress, 1);
...
void c_USimmTraceReader::handleInTxnUnitReqQTokenChgEvent(SST::Event *ev) {
...
void c_USimmTraceReader::handleInTxnUnitResPtrEvent(SST::Event* ev) {
...
void c_USimmTraceReader::handleOutTxnGenReqPtrEvent(SST::Event* ev) {
	// nothing to do here
	std::cout << __PRETTY_FUNCTION__ << " ERROR: Should not be here"
			<< std::endl;
}

void c_USimmTraceReader::handleOutTxnGenResQTokenChgEvent(SST::Event* ev) {
	// nothing to do here
	std::cout << __PRETTY_FUNCTION__ << " ERROR: Should not be here"
			<< std::endl;
}

void c_USimmTraceReader::sendTokenChg() {
...
void c_USimmTraceReader::sendRequest() {
...
void c_USimmTraceReader::readResponse() {
	if (m_txnResQ.size() > 0) {
