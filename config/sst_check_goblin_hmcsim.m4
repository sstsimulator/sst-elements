...
[
  sst_checkgoblin_hmcsim_happy="yes"
  ...
  AS_IF([test "$with_goblin_hmcsim" = "no"], [sst_checkgoblin_hmcsim_happy="no"])

  CPPFLAGS_saved="$CPPFLAGS"
  LDFLAGS_saved="$LDFLAGS"

  AS_IF([test "$sst_checkgoblin_hmcsim_happy" = "yes"], [
  ...
  AC_LANG_PUSH([C++])
  AC_CHECK_HEADER([hmc_sim.h], [], [sst_checkgoblin_hmcsim_happy="no"])
  AC_LANG_POP([C++])

  AC_CHECK_LIB([hmcsim], [hmcsim_clock],
    [GOBLIN_HMCSIM_LIB="-lhmcsim"], [sst_checkgoblin_hmcsim_happy="no"])
  ...
  AS_IF([test "x$sst_checkgoblin_hmcsim_happy" = "xyes"], [AC_DEFINE([HAVE_GOBLIN_HMCSIM],[1],[Defines whether we have the GOBLIN HMC Simulator library])])
  AM_CONDITIONAL([USE_GOBLIN_HMCSIM], [test "x$sst_checkgoblin_hmcsim_happy" = "xyes"])

  AC_MSG_CHECKING([for GOBLIN HMC Simulator library])
  AC_MSG_RESULT([$sst_checkgoblin_hmcsim_happy])
  AS_IF([test "$sst_checkgoblin_hmcsim_happy" = "no" -a ! -z "$with_goblin_hmcsim" -a "$with_goblin_hmcsim" != "no"], [$3])
  AS_IF([test "$sst_checkgoblin_hmcsim_happy" = "yes"], [$1], [$2])
])
