...
#ifndef MTGL_VOLVOLUME_H
#define MTGL_VOLVOLUME_H
...
public:
  ...
  VOL_dual(const int dsize) : u(dsize)
  {
    u = 0.0;
  }
...
public:
  ...
  lfactor(const VOL_parms& parm, const double lambda, const int iter)
  {
    ...
    case green:
      ...
      if (parm.printflag & 4) printf("      G: Consecutive Gs = %3d\n\n", cons);
    ...
    case yellow:
      ...
      if (parm.printflag & 4) printf("      Y: Consecutive Ys = %3d\n\n", cons);
    ...
    case red:
      ...
      if (parm.printflag & 4) printf("      R: Consecutive Rs = %3d\n\n", cons);
...
public:
  ...
  inline double factor(const VOL_parms& parm, const double lcost,
                       const double alpha)
  {
    if (alpha < parm.alphamin) return 1.0;
...
public:
  ...
  */
  ...
  //DEPRECATED
/*    virtual int solve_subproblem_with_mask(const VOL_dvector& dual, const VOL_dvector& rc, */
/*                              double& lcost, VOL_dvector& x, VOL_dvector& v, */
/*                              double& pcost, int **load_balance_masks, */
...
/** Starting from a initial set of lagrange multipliers in user memory, load
       them into Vol's data structures.
       @param u  (OUT) the initial dual vector
   */
...
public:
  ...
  /* int solve_with_mask(VOL_user_hooks& hooks, int **load_balance_masks, int *load_balance_totals,  */
/*                     int **load_balance_parallelize_flags, int num_load_balance_masks, const bool use_preset_dual = false); */
