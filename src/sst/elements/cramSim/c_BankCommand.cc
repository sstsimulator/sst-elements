...
void c_BankCommand::print(SST::Output *x_debugOutput, SimTime_t x_cycle) const {
    x_debugOutput->verbose(CALL_INFO, 1, 0, "[BankCommand] Cycle:%" PRIx64 "," , x_cycle);
...
void c_BankCommand::print(SST::Output *x_debugOutput,const std::string x_prefix, SimTime_t x_cycle) const {
    x_debugOutput->verbosePrefix(x_prefix.c_str(),CALL_INFO,1,0,"Cycle:%" PRIx64 " Cmd:%s seqNum: %" PRIx64 " CH:%d PCH:%d Rank:%d BG:%d B:%d Row:%d Col:%d BankId:%d\n",
    x_cycle,
