...
#ifndef __NULLENVIR_H
#define __NULLENVIR_H
...
class NullEnvir : public cEnvir
{
  ...
  protected:
      void unsupported(string from) const {
	//printf("%s: ", from.c_str());
	cout << from.c_str() << ":  ";
      throw cRuntimeError("cNullEnvir: unsupported method called");}
    ...
    virtual bool askyesno(const char *msg)  {unsupported("askyesno"); return false;}

  public:
    // constructor, destructor
    NullEnvir(int ac, char **av, cConfiguration *c) {argc=ac; argv=av; cfg=c; rng=new cMersenneTwister(); lastnum=0;}
    virtual ~NullEnvir() {delete cfg; delete rng;}
    ...
    virtual void readParameter(cPar *par)  {unsupported("readParameter");}
    virtual bool isModuleLocal(cModule *parentmod, const char *modname, int index)  {return true;}
    virtual cXMLElement *getXMLDocument(const char *filename, const char *path=NULL)  {unsupported("getXMLDocumnet"); return NULL;}
    ...
    virtual std::string gets(const char *prompt, const char *defaultreply=NULL)  {unsupported("gets"); return "";}
    ...
    // snapshot file
    virtual std::ostream *getStreamForSnapshot()  {unsupported("getstreamforsnapshot"); return NULL;}
    virtual void releaseStreamForSnapshot(std::ostream *os)  {unsupported("releasestremforsnapshot");}
