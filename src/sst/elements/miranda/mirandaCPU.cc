...
RequestGenCPU::RequestGenCPU(SST::ComponentId_t id, SST::Params& params) :
  ...
  std::string cpuClock = params.find<std::string>("clock", "2GHz");
	clockHandler = new Clock::Handler<RequestGenCPU,&RequestGenCPU::clockTick>(this);
	...
	cache_link = loadUserSubComponent<Interfaces::StandardMem>("memory", ComponentInfo::SHARE_NONE,
			timeConverter, new Interfaces::StandardMem::Handler<RequestGenCPU,&RequestGenCPU::handleEvent>(this) );
	if (!cache_link) {
	    ...
	    cache_link = loadAnonymousSubComponent<Interfaces::StandardMem>(interfaceName, "memory", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS,
                    interfaceParams, timeConverter, new Interfaces::StandardMem::Handler<RequestGenCPU,&RequestGenCPU::handleEvent>(this));
    ...
    // TODO create a generator that gets data from a port
		...
		srcLink = configureLink( "src", "50ps", new Event::Handler<RequestGenCPU,&RequestGenCPU::handleSrcEvent>(this));
		if ( NULL == srcLink ) {
