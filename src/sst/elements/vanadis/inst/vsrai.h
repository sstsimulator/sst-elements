...
template <VanadisRegisterFormat register_format>
class VanadisSIMTShiftRightArithmeticImmInstruction : public VanadisSIMTInstruction, public VanadisShiftRightArithmeticImmInstruction<register_format>
{
public:
    VanadisSIMTShiftRightArithmeticImmInstruction(
        const uint64_t addr, const uint32_t hw_thr, const VanadisDecoderOptions* isa_opts, const uint16_t dest,
        const uint16_t src_1, const int64_t immediate) :
        VanadisInstruction(addr, hw_thr, isa_opts, 1, 1, 1, 1, 0, 0, 0, 0),
        VanadisSIMTInstruction(addr, hw_thr, isa_opts, 1, 1, 1, 1, 0, 0, 0, 0),
        VanadisShiftRightArithmeticImmInstruction<register_format>(addr, hw_thr, isa_opts, dest, src_1, immediate)
    {
        ;
    }

    VanadisSIMTShiftRightArithmeticImmInstruction* clone() override
    {
        return new VanadisSIMTShiftRightArithmeticImmInstruction(*this);
    }
