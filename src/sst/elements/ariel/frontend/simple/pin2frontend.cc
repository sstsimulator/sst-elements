...
Pin2Frontend::Pin2Frontend(ComponentId_t id, Params& params, uint32_t cores, uint32_t maxCoreQueueLen, uint32_t defMemPool) :
    ...
    snprintf(tool_path, sizeof(char)*1024, "%s/fesimple.dylib", ARIEL_STRINGIZE(ARIEL_TOOL_DIR));
#else
    snprintf(tool_path, sizeof(char)*1024, "%s/fesimple.so", ARIEL_STRINGIZE(ARIEL_TOOL_DIR));
#endif
    ...
    snprintf(execute_args[0], "%s", appLauncher.c_str());
    arg++;
    ...
    for(uint32_t aa = 0; aa < launch_param_count; aa++) {
        snprintf(param_name_buffer, sizeof(char)*512, "launchparam%" PRIu32, aa);
