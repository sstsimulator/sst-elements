...
{
    m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_WAIT tid=%d wakeup \n",m_process->gettid());
    setReturnSuccess(0);
}

void VanadisFutexSyscall::memReqIsDone(bool failed )
{
    m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] memReqIsDone failed=%d \n",failed);
    switch( m_op ) {
      case FUTEX_WAIT: 
      case FUTEX_WAIT_BITSET:
        {
      ...
      case FUTEX_REQUEUE:
        {
            if ( ReadAddr == m_state ) { 
                ...
                m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_REQUEUE wakeup %d waiter\n",val);

                int numWaiters = m_process->futexGetNumWaiters( getEvent<VanadisSyscallFutexEvent*>()->getAddr() );
                m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_REQUEUE numWaiters %d\n",numWaiters);
                ...
                for ( int i = 0; i < val && i < numWaiters; i++ ) { 
                    ...
                    if ( syscall ) {
                        ++m_numWokeup;
                        m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_REQUEUE tid=%d addr=%#" PRIx64 " wakeup tid=%d\n",
                                    m_process->gettid(), getEvent<VanadisSyscallFutexEvent*>()->getAddr(), syscall->getTid());
                        dynamic_cast<VanadisFutexSyscall*>( syscall )->wakeup();
                        delete syscall;
                    }
                }
                m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_REQUEUE numWokeup %d\n",m_numWokeup);
                ...
                if ( 0 == numWaiters - m_numWokeup ) {
                    ...
                    if ( getEvent<VanadisSyscallFutexEvent*>()->getStackPtr() ) {
                        ...
                        m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_REQUEUE not tested code\n");
...
{
    m_output->verbose(CALL_INFO, 0, 0, "[syscall-futex] FUTEX_REQUEUE read val2=%d addr2=%#" PRIx64 "\n",val2,addr2);
    ...
    for ( int i = 0; i < val2 && i < numWaiters; i++ ) { 
        ...
        VanadisSyscall* syscall = m_process->findFutex( getEvent<VanadisSyscallFutexEvent*>()->getAddr() );
        if ( syscall ) {
            m_output->verbose(CALL_INFO, 0, 0,"[syscall-futex] FUTEX_REQUEUE tid=%d addr=%#" PRIx64 " move to %#" PRIx64 "\n",m_process->gettid(),
