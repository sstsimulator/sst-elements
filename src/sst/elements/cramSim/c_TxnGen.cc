...
void c_TxnGenBase::finish()
{
    ...
    output->output("Total Txns Sents: %" PRIx64 "\n", m_resReadCount + m_resWriteCount);

    output->output("Total Read-Txns Responses received: %" PRIx64 "\n", m_resReadCount);
...
bool c_TxnGenBase::sendRequest()
{
    ...
    #ifdef __SST_DEBUG_OUTPUT__
        debug->verbose(CALL_INFO,1,0,"[cycle:%" PRIu64 "] addr: 0x%lx isRead:%d seqNum:%lu\n",
