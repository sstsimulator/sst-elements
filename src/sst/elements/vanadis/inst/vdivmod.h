...
public:
    ...
    void printToBuffer(char* buffer, size_t buffer_size) override
    {
        snprintf(
            buffer, buffer_size,
            "DIVREM%c q: %5" PRIu16 " r: %" PRIu16 " <- %" PRIu16 " \\ %" PRIu16 ", (phys: q: %" PRIu16 " r: %" PRIu16
            " r: %" PRIu16 " %" PRIu16 " )\n",
    ...
    void execute(SST::Output* output, VanadisRegisterFile* regFile) override
    {
        ...
        output->verbose(
            CALL_INFO, 16, 0,
            "Execute: (addr=%p) DIVREM%c q: %" PRIu16 " r: %" PRIu16 " <- %" PRIu16 " \\ %" PRIu16 " (phys: q: %" PRIu16
            " r: %" PRIu16 " %" PRIu16 " %" PRIu16 ")\n",
            (void*)getInstructionAddress(), perform_signed ? ' ' : 'U', isa_int_regs_out[0], isa_int_regs_out[1],
        ...
        if ( perform_signed ) {
            ...
            case VanadisRegisterFormat::VANADIS_FORMAT_INT64:
            {
                ...
                if ( 0 == src_2 ) {
                    ...
                    output->verbose(
                        CALL_INFO, 16, 0,
                        "--> Execute: (detailed, unsigned, DIVREM64) %" PRIu64 " / %" PRIu64 " = (q: %" PRIu64
                        ", r: %" PRIu64 ")\n",
                        src_1, src_2, quo, mod);
#endif
            ...
            case VanadisRegisterFormat::VANADIS_FORMAT_INT32:
            {
                ...
                if ( 0 == src_2 ) {
                    ...
                    output->verbose(
                        CALL_INFO, 16, 0,
                        "--> Execute: (detailed, unsigned, DIVREM32) %" PRIu32 " / %" PRIu32 " = (q: %" PRIu32
                        ", r: %" PRIu32 ")\n",
                        src_1, src_2, quo, mod);
#endif
