...
verona::verona(ComponentId_t id, Params_t& params) :
  Component(id) {
  ...
  if( params.find("clock") != params.end() ) {
	string clock_rate = params[ "clock" ];
  	...
  	// register the verona clock for generating addresses
  	registerClock( clock_rate, new Clock::Handler<verona>(this, &verona::tick ) );
	...
	if(output_level > 0) {
  	...
  	// register the verona clock for generating addresses
  	registerClock( "2GHz", new Clock::Handler<verona>(this, &verona::tick ) );

  }

  // configure link to cache components
  cache_link = configureLink( "cache_link", new Event::Handler<verona>(this,
                                &verona::handleEvent) );
  assert(cache_link);
...
read_trace_return verona::readNextRequest(memory_request* req) {
	if(feof(trace_input)) {
...
void verona::handleEvent(Event *ev)
{
...
void verona::createPendingRequest(memory_request mem_req) {
	bool is_read = true;
...
BOOST_CLASS_EXPORT(verona)

static Component*
create_verona(SST::ComponentId_t id, 
                  SST::Component::Params_t& params)
{
    return new verona( id, params );
}
