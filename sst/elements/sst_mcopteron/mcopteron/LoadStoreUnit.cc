...
///
LoadStoreUnit::LoadStoreUnit(unsigned int numberSlots, 
...
void LoadStoreUnit::notify(void *obj)
{
  ...
  if (i < numSlots) {
    if (slots[i].type == LSType::LOAD)
      slots[i].token->cancelInstruction(0);
    // 128-bit stores take up two slots
    if((slots[i].type == LSType::STORE) && (slots[i].token->getType()->getMaxOpSize() == 128 ))
      numFilled-=2;
    else 
      numFilled--;
    // clear slot
...
void LoadStoreUnit::updateStatus(CycleCount currentCycle)
{
  ...
  // first loop
  for (i=0; i < numSlots; i++) {
    if (slots[i].type == LSType::EMPTY )
      continue;
    if (slots[i].token && slots[i].token->wasCanceled()) {
      // release slots
      if(slots[i].token->getType()->getMaxOpSize() == 128 &&  slots[i].type==LSType::STORE)
        numFilled-=2;
      else
        numFilled--;
    ...
    if (slots[i].satisfiedCycle > 0 && slots[i].satisfiedCycle <= currentCycle) {
      ...
      // same with a store
      if (slots[i].type == LSType::STORE)
        slots[i].token->storeSatisfiedAt(currentCycle);
      // now empty the occupied entries from the LSQ
      if(slots[i].token && slots[i].token->getType()->getMaxOpSize() == 128 &&  slots[i].type == LSType::STORE)
        numFilled-=2;
      else
        numFilled--;
      ...
      slots[i].type   = LSType::EMPTY; // clear this record
  ...
  for (i=0; i < numSlots && numOps < maxMemOpsPerCycle ; i++) {
    //TODO: slots[i].type == LSType::EMPTY ???
    if (slots[i].type != LSType::STORE && slots[i].type != LSType::LOAD)
      continue;
    if (Debug>2) 
      fprintf(stderr,"LSQ slot %d: token %llu satCyc %llu addrRdy %u\n",
              i, slots[i].token->instructionNumber(), slots[i].satisfiedCycle,
    ...
    if (slots[i].token && slots[i].type == LSType::STORE && slots[i].satisfiedCycle == 0 && canServeStores) {
    ...
    // cycle (it is still 0) and this token's address is now read and we 
    // haven't already performed the maximum memory ops this cycle, then serve it
    if (slots[i].token && slots[i].type == LSType::LOAD && slots[i].satisfiedCycle == 0 && 
        slots[i].token->addressIsReady() && numOps < maxMemOpsPerCycle) {

      slots[i].satisfiedCycle = memoryModel->serveLoad(currentCycle,0,0,slots[i].token);
      if (Debug>1)
        fprintf(stderr, "LSQ: token %llu will be satisfied at %llu\n",
                slots[i].token?slots[i].token->instructionNumber():0, 
                slots[i].satisfiedCycle);
