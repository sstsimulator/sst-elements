...
####
    ...
    def memH_test_template(self, omp_app):
        ...
        testDataFileName="test_dirnoncacheable_openMP_{0}".format(omp_app)
        reffile = "{0}/refFiles/test_OMP_noncacheable_{1}.out".format(test_path, omp_app)
        outfile = "{0}/{1}.out".format(outdir, testDataFileName)
        errfile = "{0}/{1}.err".format(outdir, testDataFileName)
        mpioutfiles = "{0}/{1}.testfile".format(outdir, testDataFileName)

        sdlfile = "{0}/openMP/dir-non-cacheable-8cores-2nodes.py".format(test_path)
        testtimeout = 120

        # Set the environment path to the omp application used by the SDL file
        # and verify it exists
        ompapppath = "{0}/openMP/{1}/{1}".format(test_path, omp_app)
        self.assertTrue(os.path.isfile(ompapppath), "openMP dirnoncacheable Test; ompfile {0} - not found".format(ompapppath))
        ...
        self.assertTrue(outtestresult, "openMP dirnoncacheable Test - Cannot find string \"{0}\" in output file {1}".format(grepstr, outfile))
            ...
            for line in f.readlines():
                ...
                if (testline != "") and ("Simulation is complete" not in testline):
                    ...
                    if rtn.result() == 0:
                        ...
                        log_failure("FAILURE: openMP dirnoncacheable Test failed running cmdline {0} - grepping outfile {1}".format(cmd, outfile))
