	...
	else
	{
		...
		if(emulate_faults)
		{
			if(WSR_COUNT[pw_id]==4) {
				...
				//std::cerr<<"Sending a new PGD request with address1 "<<std::hex<<dummy_add<<" vaddress: " << addr <<std::endl;
			}
			else if(WSR_COUNT[pw_id]==3) {
				dummy_add = (*PGD)[(addr/page_size[3])%512];// + (addr/page_size[3])%512;
				...
				//std::cerr<<"Sending a new PUD request with address1 "<<std::hex<<dummy_add<<" vaddress: " << addr << " PUD page: " << (*PGD)[addr/page_size[3]]<<" offset: "<<(addr/page_size[3])<<"("<<(addr/page_size[3])%512<<")"<<std::endl;
				pud_accesses->addData(1);
			}
			else if(WSR_COUNT[pw_id]==2) {
				dummy_add = (*PUD)[(addr/page_size[2])%(512*512)];// + (addr/page_size[2])%512;
				...
				//std::cerr<<"Sending a new PMD request with address1 "<<std::hex<<dummy_add<<" vaddress: " << addr<<" PMD page: " << (*PUD)[addr/page_size[2]]<<" offset: "<<(addr/page_size[2])<<"("<<(addr/page_size[2])%512<<")"<<std::endl;
				pmd_accesses->addData(1);
			}
			else if(WSR_COUNT[pw_id]==1) {
				uint64_t offset = (uint64_t)512*512*512;
				dummy_add = (*PMD)[(addr/page_size[1])%offset];// + (addr/page_size[1])%512;
				...
				//std::cerr<<"Sending a new PTE request with address1 "<<std::hex<<dummy_add<<" vaddress: " << addr<<" PTE page: " << (*PMD)[addr/page_size[1]]<<" offset: "<<(addr/page_size[1])<<"("<<(addr/page_size[1])%512<<")"<<std::endl;
		...
		SST::Event * ev = e;
		//std::cerr << Owner->getName().c_str() << " core: " << coreId <<" ptw address: " << e->getAddr() << " base address: " << e->getBaseAddr() <<" vaddress: " << addr<< " line size: " << line_size << std::endl;
		WSR_COUNT[pw_id]--;
			...
			{	
				...
				if(to_mem!=NULL)
				{
					...
					if(emulate_faults)
					{
						if(k==4) {
							dummy_add = (*CR3);
							//std::cerr<<"Sending a new PGD request with address "<<std::hex<<dummy_add<<" vaddress: " << addr<<std::endl;
							pgd_accesses->addData(1);
						}
						else if(k==3) {
							dummy_add = (*PGD)[(addr/page_size[3])%512];// + (addr/page_size[3])%512;
							...
							//std::cerr<<"Sending a new PUD request with address "<<std::hex<<dummy_add<<" vaddress: " << addr<<" PUD page: " << (*PGD)[addr/page_size[3]]<<" offset: "<<(addr/page_size[3])<<"("<<(addr/page_size[3])%512<<")"<<std::endl;
							pud_accesses->addData(1);
						}
						else if(k==2) {
							dummy_add = (*PUD)[(addr/page_size[2])%(512*512)];// + (addr/page_size[2])%512;
							...
							//std::cerr<<"Sending a new PMD request with address "<<std::hex<<dummy_add<<" vaddress: " << addr<<" PMD page: " << (*PUD)[addr/page_size[2]]<<" offset: "<<(addr/page_size[2])<<"("<<(addr/page_size[2])%512<<")"<<std::endl;
							pmd_accesses->addData(1);
						}
						else if (k==1) {
							uint64_t offset = (uint64_t)512*512*512;
							...
							//std::cerr<<"Sending a new PTE request with address "<<std::hex<<dummy_add<<" vaddress: " << addr<<" PTE page: " << (*PMD)[addr/page_size[1]]<<" offset: "<<(addr/page_size[1])<<"("<<(addr/page_size[1])%512<<")"<<std::endl;
							pte_accesses->addData(1);
						}
						else
							output->fatal(CALL_INFO, -1, "MMU: PTW DANGER!!\n");
