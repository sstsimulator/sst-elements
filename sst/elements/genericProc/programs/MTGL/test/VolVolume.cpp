                          ...
                          const VOL_dvector& dual_ub)
{
  ...
  for ( int i = 0; i < nc; ++i )
  {
    double b = ((v[i] > 0.0 && dual_ub[i] != 0.0) * v[i]) +
               ((v[i] < 0.0 && dual_lb[i] != 0.0) * -v[i]);
    if (b > _viol) _viol = b; // mta compiler takes this reduction out of the loop
  }
  viol = _viol;
/****************** deprecated code that isn't mta-friendly ****************
   for ( int i = 0; i < nc; ++i ) {
               ...
               const VOL_dvector& v)
{
  ...
  for (i = 0; i < nc; ++i)
  {
    double b = ((v[i] > 0.0 && u[i] < dual_ub[i]) ||
                (v[i] < 0.0 && u[i] > dual_lb[i])) * v[i] * v[i];
    if (b > 0.0) viol += b;
  }
/*******************deprecated, non-mta-friendly code*********************
   for (i = 0; i < nc; ++i) {
...
*************************************************************************/
  ...
  for (i = 0; i < nc; ++i)
  {
    ...
    {
      u[i] += stp * v[i];
      if (u[i] < dual_lb[i]) u[i] = dual_lb[i];
...
VOL_dual::ascent(const VOL_dvector& v, const VOL_dvector& last_u) const
{
  ...
  int i;
  double asc = 0.0;
  for (i = 0; i < nc; ++i) asc += v[i] * (u[i] - last_u[i]);
  return asc;
}
...
VOL_problem::read_params(const char* filename)
{
  ...
  while (fgets(s, 100, infile))
  {
    ...
    if (ss.find("temp_dualfile") == 0)
    {
      ...
      std::memcpy(parm.temp_dualfile, sss.c_str(), sss.length());
...
VOL_problem::set_default_parm()
{
  ...
  parm.minimum_rel_ascent = 0.01;
  parm.ascent_first_check = 500;
  parm.ascent_check_invl = 100;
  parm.maxsgriters = 2000;
  parm.printflag = 0;
  parm.printinvl = 5;
  parm.heurinvl = 100;  // INT_MAX;   // 100; changed 070909 JWB
...
// DEPRECATED
// int
// VOL_problem::solve_with_mask(VOL_user_hooks& hooks, int **load_balance_masks,
...
//    retval = hooks.solve_subproblem_with_mask(dual.u, rc, dual.lcost,
//                                              primal.x, primal.v, primal.value,
                        ...
                        const VOL_dual& dual) const
{
  ...
  if (prod.norm + prod.hh - 2.0 * prod.vh > 0.0) alpha_fb = (prod.hh - prod.vh) / (prod.norm + prod.hh - 2.0 * prod.vh);
  else
    alpha_fb = alpha;

  if (alpha_fb > alpha) alpha_fb = alpha;
  if (alpha_fb < a_asc) alpha_fb = a_asc;
  if (alpha_fb > 1.0) alpha_fb = alpha;
  if (alpha_fb < 0.0) alpha_fb = alpha / 10.0;
