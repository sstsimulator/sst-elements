...
#include "AppAll2All.h"

int AppAll2All::accessSent = 0;
int AppAll2All::notifySent = 0;

int AppAll2All::notifyRcvd = 0;
int AppAll2All::accessRcvd = 0;

AppAll2All::AppAll2All(int i, int n, DRAM_Cfg* cfg) :
	Application(i, n, cfg) {

	init();
}

void AppAll2All::init() {
...
AppAll2All::~AppAll2All() {

}

simtime_t AppAll2All::process(ApplicationData* pdata) {

	return pow(10.0, -9);
}

ApplicationData* AppAll2All::getFirstMsg() {
	return sending(NULL);
}

ApplicationData* AppAll2All::sending(ApplicationData* a) {
	...
	memory_label: if (goMemory) {
		if (useioplane) {
			if (next >= dramcfg->getNumDRAM()) {
				...
				adata->setType((adata->getDestId() == id) ? DM_writeLocal : DM_writeRemote);
...
ApplicationData* AppAll2All::dataArrive(ApplicationData* pdata) {
