...
Samba::Samba(SST::ComponentId_t id, SST::Params& params): Component(id) {
	...
	for(uint32_t i = 0; i < core_count; ++i) {
		...
		SST::Link * link2 = configureLink(link_buffer, "0ps", new Event::Handler<TLBhierarchy,&TLBhierarchy::handleEvent_CPU>(TLB[i]));
		cpu_to_mmu[i] = link2;



		SST::Link * link = configureLink(link_buffer2, "0ps", new Event::Handler<TLBhierarchy,&TLBhierarchy::handleEvent_CACHE>(TLB[i]));
		mmu_to_cache[i] = link;
		...
		if(self==0)
			link3 = configureLink(link_buffer3, new Event::Handler<PageTableWalker,&PageTableWalker::recvResp>(TLB[i]->getPTW()));
		else
			link3 = configureSelfLink(link_buffer3, std::to_string(page_walk_latency)+ "ns", new Event::Handler<PageTableWalker,&PageTableWalker::recvResp>(TLB[i]->getPTW()));
		...
		if(emulate_faults==1)
		{
			...
			event_link = configureSelfLink(link_buffer, "1ns", new Event::Handler<PageTableWalker,&PageTableWalker::handleEvent>(TLB[i]->getPTW()));
	...
	registerClock( cpu_clock, new Clock::Handler<Samba,&Samba::tick>(this) );
