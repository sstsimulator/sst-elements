...
struct cr_args
{
    ...
    cr_args(const double * const V1, const double * const V2) :
	v1(V1), v2(V2) {}
};
static void cr_loop(qthread_t *me, const size_t startat, const size_t stopat, void * arg, void *ret)
{
    const double * const v1(((struct cr_args*)arg)->v1);
    const double * const v2(((struct cr_args*)arg)->v2);
		     ...
		     const double * const v2, double * const residual)
{
  ...
  extern int tcount;
  if (tcount > 0) {
      const struct cr_args args(v1, v2);
      qt_loopaccum_balance(0, n, sizeof(double), &local_residual, cr_loop, (void*)&args, qt_dbl_max_acc);
  } else
#endif
  for (int i=0; i<n; i++) {
    double diff = fabs(v1[i] - v2[i]);
