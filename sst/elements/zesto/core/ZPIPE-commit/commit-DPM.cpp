...
void core_commit_DPM_t::step(void)
{
  ...
  for(commit_count=0;commit_count<knobs->commit.width;commit_count++)
  {
...
#ifdef ZDEBUG
      fprintf(stdout,"\n[%lld][Core%d](Commited)",core->sim_cycle,core->id );
      md_print_insn(Mop,stdout);
#endif
    ...
    else
    {
      ...
      if(uop->decode.is_load)
      {
		if(core->exec->has_load_arrived(uop->alloc.LDQ_index) == false)		
		{
			if(load_stall_on == false)
			{
				stall_start_time = core->sim_cycle;
			}
			ZESTO_STAT(core->stat.commit_load_stalls++;)
			load_stall_on=true;
		}
      }
    ...
    if (core->sim.trace_limit && (core->stat.eio_commit_insn >= core->sim.trace_limit))
    {
