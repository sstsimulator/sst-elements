...
public:
    VanadisLoadInstruction(const uint64_t addr, const uint32_t hw_thr, const VanadisDecoderOptions* isa_opts,
                           const uint16_t memAddrReg, const int64_t offst, const uint16_t tgtReg,
                           const uint16_t load_bytes, const bool extend_sign, const VanadisMemoryTransaction accessT,
                           VanadisLoadRegisterType regT)
        : VanadisInstruction(addr, hw_thr, isa_opts, 1, regT == LOAD_INT_REGISTER ? 1 : 0, 1,
                             regT == LOAD_INT_REGISTER ? 1 : 0, 0, regT == LOAD_FP_REGISTER ? 1 : 0, 0,
                             regT == LOAD_FP_REGISTER ? 1 : 0),
          offset(offst), load_width(load_bytes), signed_extend(extend_sign), memAccessType(accessT), regType(regT) {
    ...
    virtual void computeLoadAddress(SST::Output* output, VanadisRegisterFile* regFile, uint64_t* out_addr,
                                    uint16_t* width) {
        ...
        case LOAD_INT_REGISTER: {
            output->verbose(CALL_INFO, 16, 0,
                            ...
                            getInstructionAddress(), isa_int_regs_in[0], phys_int_regs_in[0], offset,
