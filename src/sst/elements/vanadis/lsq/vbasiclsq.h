    ...
    protected:
        ...
        void issueLoad(VanadisLoadInstruction* load_ins, uint64_t load_address, uint64_t load_width) {
            ...
            if(LIKELY(! load_ins->trapsError())) {
                ...
                output->verbose(CALL_INFO, 16, VANADIS_DBG_LSQ_LOAD_FLG, "-----> ins: 0x%" PRI_ADDR " / thr: %" PRIu32 " processed and requests sent to memory system. numRequests=%d\n",
        ...
        virtual bool sendLoadReq(VanadisLoadInstruction* load_ins)
        {
            ...
            output->verbose(CALL_INFO, 16, 0, " (ScalarLSQ) -> load ins: 0x%" PRI_ADDR " / thr: %" PRIu32 "\n",
            ...
            output->verbose(CALL_INFO, 16, 0, " (ScalarLSQ) -> load ins: 0x%" PRI_ADDR " / thr: %" PRIu32 " result=%s #load_address=%d #load_widths=%d...\n",
        ...
        virtual bool sendStoreReq(VanadisInstruction* store_ins_temp)
        {
            ...
            else
            {
                ...
                output->verbose(CALL_INFO, 16, 0, " (ScalarLSQ) -> queue front is store: ins: 0x%" PRI_ADDR " / thr: %" PRIu32 " has issued so will process...\n",
        ...
        {      
            ...
            output->verbose(CALL_INFO, 16, 0, "---> computeLoadAddress for sw_thr: %" PRIu64 "\n",sw_thr);
            load_ins->computeLoadAddress(output, hw_thr_reg, &load_address, &load_width);
            output->verbose(CALL_INFO, 16, 0, "---> computeLoadAddress for 0x%" PRI_ADDR " / sw_thr: %" PRIu64 "\n",
                load_address, sw_thr);
            ...
            else 
            {
                ...
                if(UNLIKELY(checkStoreConflict(load_ins->getHWThread(), load_address, load_width))) 
                {
                    ...
                    output->verbose(CALL_INFO, 16, 0, "---> issue LOAD failed: store conflict\n");
                    return false;
                ...
                else if((load_ins->getTransactionType() == MEM_TRANSACTION_LLSC_LOAD) || (load_ins->getTransactionType() == MEM_TRANSACTION_LOCK)) 
                {
                    ...
                    {
                        output->verbose(CALL_INFO, 16, 0, "---> issue LLSC/LOCK LOAD failed: store pending\n");
                        return false;
                    } 
                    else 
                    {
        ...
        VanadisBasicStorePendingEntry* store_process(uint32_t sw_thr,VanadisStoreInstruction* store_ins,uint64_t* store_address_last, uint8_t* trap_error)
        {
            ...
            *trap_error = 0;
            output->verbose(CALL_INFO, 16, 0, "_SIMT --> computeStoreAddress for sw_thr: %" PRIu64 "\n",sw_thr);
            ...
            if ((*store_address_last != store_address) || (*store_address_last ==0))
            {
                ...
                output->verbose(CALL_INFO, 16, 0, "----> ins_addr: 0x%" PRI_ADDR " store address: 0x%" PRI_ADDR " store_address_last: 0x%" PRI_ADDR " width: %" PRIu16 " sw_thr: %" PRIu32 " hw_thr: %" PRIu32 "\n",
            ...
            {
                output->verbose(CALL_INFO, 16, 0, "----> pending_store=null address: 0x%" PRI_ADDR " store_address_last: 0x%" PRI_ADDR " width: %" PRIu16 " sw_thr: %" PRIu32 " hw_thr: %" PRIu32 "\n",
                    store_address, *store_address_last, store_width, sw_thr, store_ins->getHWThread());
                return nullptr;
            }
...
class VanadisBasicLoadStoreQueueSIMT: public VanadisBasicLoadStoreQueue
{
    public:
    SST_ELI_REGISTER_SUBCOMPONENT(VanadisBasicLoadStoreQueueSIMT, "vanadis", "VanadisBasicLoadStoreQueueSIMT",
                                          SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                          "Implements a basic load-store queue with write buffer for use with the SST standardInterface",
                                          SST::Vanadis::VanadisLoadStoreQueue)
    ...
    VanadisBasicLoadStoreQueueSIMT(ComponentId_t id, Params& params, int coreid, int hwthreads):
    VanadisBasicLoadStoreQueue(id, params, coreid, hwthreads)
    {
        ;
        std_mem_handlers = new VanadisBasicLoadStoreQueueSIMT::StandardMemHandlersSIMT(this, output);

        // memInterface = loadUserSubComponent<Interfaces::StandardMem>(
        //     "memory_interface", ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, getTimeConverter("1ps"),
        //     new StandardMem::Handler<SST::Vanadis::VanadisBasicLoadStoreQueueSIMT>(
        //         this, &VanadisBasicLoadStoreQueue::processIncomingDataCacheEvent));
        
    }
    
    protected:
        bool sendLoadReq(VanadisLoadInstruction* load_ins_temp) override
        {
            
            if (!load_ins_temp->getIsSIMT())
            {
                output->verbose(CALL_INFO, 16, 0, 
                "In sendLoadReq (scalarLSQ) hw_thr:%d\n", load_ins_temp->getHWThread());
                return VanadisBasicLoadStoreQueue::sendLoadReq(load_ins_temp);
            }
            VanadisSIMTLoadInstruction* load_ins = dynamic_cast<VanadisSIMTLoadInstruction*>(load_ins_temp);
            output->verbose(CALL_INFO, 16, 0, 
                "In sendLoadReq (SIMTLSQ) hw_thr:%d\n", load_ins->getHWThread());
            std::vector<uint64_t> load_address_simt;
            std::vector<uint16_t> load_width_simt;
            bool result = false;
            uint64_t thr = load_ins->getHWThread();
            for (uint64_t sw_thr = thr; sw_thr < (thr + WARP_SIZE); sw_thr++)
            {
                ...
                result = load_process(sw_thr,load_ins, load_address_simt, load_width_simt);
                if((result==false) || (load_address_simt.size()==0))
                {
                    load_ins->setNumLoads(0);
                    break; 
                }
            ...
            output->verbose(CALL_INFO, 16, 0, "_SIMT--> load ins: 0x%" PRI_ADDR " / thr: %" PRIu32 " result=%s #load_address=%d #load_widths=%d...\n",
                    load_ins->getInstructionAddress(), load_ins->getHWThread(), (result==true) ? "success": "fail", load_address_simt.size(), load_width_simt.size());

            if(LIKELY((load_address_simt.size()>0) & (load_width_simt.size()>0)))
            {
                if(result)
                {
                    if((load_address_simt.size()==WARP_SIZE) || (load_address_simt.size()<=(hw_threads))) //TODO: Is the hw_threads condition ok?
                    {
                        for(uint32_t sw_thr = 0; sw_thr< WARP_SIZE; sw_thr++)
                        {
                            if((sw_thr+thr) >= hw_threads)
                            {
                                break;
                            }
                            load_ins->setSWThread(sw_thr+thr);
                            issueLoad(load_ins, load_address_simt[sw_thr], load_width_simt[sw_thr]);
                        }
                        return true;
                    }
                    else
        ...
        bool sendStoreReq(VanadisInstruction* store_ins_temp) override
        {
            
            if (!store_ins_temp->getIsSIMT())
            {
                printf("Send store req is scalar\n");
                return VanadisBasicLoadStoreQueue::sendStoreReq(store_ins_temp);
            }

            VanadisSIMTStoreInstruction* store_ins = dynamic_cast<VanadisSIMTStoreInstruction*>(store_ins_temp);
            output->verbose(CALL_INFO, 16, 0, 
                "In sendstoreReq (SIMTLSQ) hw_thr:%d\n", store_ins->getHWThread());
            std::vector<VanadisBasicStorePendingEntry*> store_issues;
            uint64_t store_address_last = 0;
            bool result = false;
            uint8_t trap_error = 0;
            uint64_t thr = store_ins->getHWThread();
            for (uint64_t sw_thr = thr; sw_thr < (thr + WARP_SIZE); sw_thr++)
            {
                if(sw_thr >= hw_threads)
                {
                ...
                VanadisBasicStorePendingEntry* new_pending_store = store_process(sw_thr,store_ins, &store_address_last, &trap_error);
                if(trap_error)
                {
                    store_ins->setNumStores(0);
                    store_issues.clear();
                ...
                if(new_pending_store != nullptr)
                {
                    store_issues.push_back(new_pending_store);
                }
                else
                {
                    if(store_issues.size()==0)
                    {
                        output->fatal(CALL_INFO, -1,
                            "_SIMT--> Logic failure, Store issues empty ins: 0x%" PRI_ADDR " / thr: %" PRIu32 "\n",store_ins->getInstructionAddress(), store_ins->getHWThread());
                        return false;
                    }
                    else
                    {
                        store_issues.front()->addThr(sw_thr);
                    }
                    
                }                    
            }
            
            if((store_issues.size()==0) &(trap_error==0))
            {
                output->verbose(CALL_INFO, 16, 0, 
                "_SIMT--> issue STORE failed: not enough load addresses hw_thr:%d, num store addresses = %d\n", store_ins->getHWThread(),store_issues.size());
                return false;
                
            }
            
            for(int i=0; i<store_issues.size();i++)
            {
                stores_pending[store_ins->getHWThread()].push_back(store_issues[i]);
                stores_pending_size++;
                output->verbose(CALL_INFO, 16, 0, 
                "In sendstoreReq (SIMTLSQ) hw_thr:%d sending store to pending list i=%d\n", store_ins->getHWThread(), i);
                store_ins->incrementNumStores();
            }
            return true;
        }

        void getStoreTarget(VanadisBasicStorePendingEntry* store_entry, VanadisStoreInstruction* store_ins_temp, uint16_t* target_thread, uint16_t* reg) override
        {
            if(!store_ins_temp->getIsSIMT())
            {
                VanadisBasicLoadStoreQueue::getStoreTarget(store_entry, store_ins_temp,target_thread, reg );
                return;
                
            }
            printf("SIMT getStoreTarget\n");
            VanadisSIMTStoreInstruction* store_ins = dynamic_cast<VanadisSIMTStoreInstruction*>(store_ins_temp);
            uint16_t thr = store_entry->getSWThr();
            uint16_t regtmp = store_ins->getValueRegisterType() == STORE_FP_REGISTER ? store_ins->getPhysFPRegIn(0,thr) : store_ins->getPhysIntRegIn(1,thr);
            *target_thread = thr;
            *reg = regtmp;
            printf("SIMT getStoreTarget\n");
        }

        void addLoadRequest(VanadisLoadInstruction* load_ins_temp,VanadisBasicLoadPendingEntry* load_entry, StandardMem::Request* load_req) override
        {
            if(!load_ins_temp->getIsSIMT())
            {
                VanadisBasicLoadStoreQueue::addLoadRequest(load_ins_temp,load_entry, load_req);
                return;
            }
            VanadisSIMTLoadInstruction* load_ins = dynamic_cast<VanadisSIMTLoadInstruction*>(load_ins_temp);
            load_entry->addRequest(load_req->getID(), load_ins->getSWThread());
            load_ins->incrementNumLoads();
        }

        class StandardMemHandlersSIMT : public VanadisBasicLoadStoreQueue::StandardMemHandlers
        {
            public:
                friend class VanadisBasicLoadStoreQueueSIMT;
                StandardMemHandlersSIMT(VanadisBasicLoadStoreQueueSIMT* lsq, SST::Output* output) :StandardMemHandlers(lsq, output), lsq(lsq)
                        {;}

                virtual ~StandardMemHandlersSIMT() {;}

                void copyLoadResp(VanadisLoadInstruction* load_ins_temp, uint16_t* target_reg,
                    uint16_t* target_isa_reg,
                    uint32_t* target_thread,VanadisBasicLoadPendingEntry* load_entry,
                    uint8_t fp) override
                {
                    if (!load_ins_temp->getIsSIMT())
                    {
                        return VanadisBasicLoadStoreQueue::StandardMemHandlers::copyLoadResp(load_ins_temp, target_reg,
                    target_isa_reg,target_thread,load_entry,fp);
                    }
                    
                    out->verbose(CALL_INFO, 16, VANADIS_DBG_LSQ_STORE_FLG, " (SIMTLSQ) -> copyLoadResp\n");
                    VanadisSIMTLoadInstruction* load_ins = dynamic_cast<VanadisSIMTLoadInstruction*>(load_ins_temp);
                    *target_thread = load_entry->getSWThr();
                    if(fp==true)
                    {
                        *target_isa_reg =  load_ins->getISAFPRegOut(0);
                        *target_reg = load_ins->getPhysFPRegOut(0,*target_thread);
                    }
                    else
                    {
                        *target_isa_reg =  load_ins->getISAIntRegOut(0);
                        *target_reg = load_ins->getPhysIntRegOut(0,*target_thread);
                    }
                }

                void processLLSC(StandardMem::WriteResp* ev, VanadisStoreInstruction* store_ins_temp,VanadisBasicStorePendingEntry* store_entry) override
                {
                    if (!store_ins_temp->getIsSIMT())
                    {
                        return VanadisBasicLoadStoreQueue::StandardMemHandlers::processLLSC(ev, store_ins_temp, store_entry);
                    }
                    out->verbose(CALL_INFO, 16, VANADIS_DBG_LSQ_STORE_FLG, " (SIMTLSQ) -> processLLSC\n");
                    VanadisSIMTStoreInstruction* store_ins = dynamic_cast<VanadisSIMTStoreInstruction*>(store_ins_temp);
                    uint16_t sw_thr = store_entry->getSWThr();
                    uint16_t coalesced_threads = store_entry->getNumCoalescedThreads();
                    VanadisStoreConditionalInstruction* store_cond_ins = dynamic_cast<VanadisStoreConditionalInstruction*>(store_ins);
                    // TODO_Anu Dynamic cast to a SIMT store instruction?
                    out->verbose(CALL_INFO, 9, VANADIS_DBG_LSQ_STORE_FLG,
                                        "---> LSQ LLSC-STORE num_coalesced_threads=%d");
                    if(coalesced_threads==0)
                    {
                        out->fatal(CALL_INFO, -1, "Unable to find any threads in LLSC_STORE entry, logic failure.\n");
                    }
                    for(uint16_t i=0; i<coalesced_threads; i++)
                    {
                        uint16_t sw_thr = store_entry->getCoalescedSwThr(i);
                        uint16_t value_reg = store_ins->getPhysIntRegOut(0,sw_thr);
                        if (ev->getSuccess()) 
                        {
                        int64_t success_result = store_cond_ins->getResultSuccess();
                        out->verbose(CALL_INFO, 9, VANADIS_DBG_LSQ_STORE_FLG,
                                        "---> LSQ LLSC-STORE rt: %" PRIu64 " <- %" PRIu16 " (success) sw_thr:%d \n",
                                        success_result, value_reg, sw_thr);
                        lsq->registerFiles->at(sw_thr)->setIntReg<int64_t>(value_reg,
                            success_result);
                        } 
                        else 
                        {
                            int64_t failure_result = store_cond_ins->getResultFailure();

                            out->verbose(CALL_INFO, 9, VANADIS_DBG_LSQ_STORE_FLG, "---> LSQ LLSC-STORE rt: %" PRIu64 " <- %" PRIu16 " (failed)  sw_thr:%d  \n",
                                            failure_result, value_reg, sw_thr);
                            lsq->registerFiles->at(sw_thr)->setIntReg<uint64_t>(value_reg,
                                failure_result);
                        }
                    }
                }
                VanadisBasicLoadStoreQueueSIMT* lsq;
        };
}; // namespace Vanadis
} // namespace SST
}
