...
/// @brief Constructor 
///
McOpteron::McOpteron():rand(&genrand_real1,&init_genrand)
{
  totalInstructions = 0;
  currentCycle = 0;
  ...
  fetchedBuffer=0;
  decodeBuffer=0;
  // initialize cpi stack to zero's
  cpiStack = new CPIStack; 
  cpiStack->brMiss = cpiStack->iCacheMiss =  cpiStack->iTLBMiss = cpiStack->fetchBuff = cpiStack->fetchBuffEmpty = cpiStack->decode = cpiStack->dispatch = 0.0;
  cpiStack->insLatency = cpiStack->dataDep =  cpiStack->dTLBMiss = cpiStack->L1 = cpiStack->L2 = cpiStack->L3 = cpiStack->mem = 0.0;
  cpiStack->decodeStall = cpiStack->totalBackend = cpiStack->ROBEmpty = cpiStack->ROBNotFull = cpiStack->ROBNotReady = 0.0;
                    ...
                    string FetchSizeFilename, string TransProbFilename)
{
  ...
  if(Debug>2){
    cerr<<"Random Number Stream (one massive line)"<<endl;
    for(int dr=0;dr<100;++dr)
      cerr<<" "<<genRandomProbability();
    cerr<<endl;
  }
  ...
  if (config->useDomain("ReorderBuffer")) {
    ...
    reorderBuffer = new ReorderBuffer(numslots, numretire, cpiStack);
    fakeIBuffer = new ReorderBuffer(100, 1, cpiStack); // for retiring fake LEAs
  ...
  if (config->useDomain("Architecture")) {
    ...
    numInstrQueues = numintqs+numfloatqs; 
    ...
    for (i=0; i < numintqs; i++) {
      ...
      while (v) {
        fu = 0;
        if (!strcmp(v,"AGU")) 
          fu = new FunctionalUnit(FunctionalUnitType::AGU, "Regular AGU", i+1);
        else if (!strcmp(v,"ALU")) 
          fu = new FunctionalUnit(FunctionalUnitType::ALU1, "Regular ALU", i+1);
        else if (!strcmp(v,"ALUSP")) 
          fu = new FunctionalUnit(FunctionalUnitType::ALU2, "Special ALU", i+1);
        else if (!strcmp(v,"ALUMULT")) 
          fu = new FunctionalUnit(FunctionalUnitType::ALU0, "Multiply ALU", i+1);
        if (!fu) {
    ...
    for (; i < numintqs+numfloatqs; i++) {
      //QType qtype = Qtype::FLOAT;
      iq = new InstructionQueue(QType::FLOAT, qnames[i], i+1, qsizes[i], qrates[i]);
      if (iqTail) {
        iqTail->setNext(iq);
        iqTail = iq;
      } else {
      ...
      while (v) {
        fu = 0;
        if (!strcmp(v,"FADD")) 
          fu = new FunctionalUnit(FunctionalUnitType::FADD, "Float Adder", i+1);
        else if (!strcmp(v,"FMUL")) 
          fu = new FunctionalUnit(FunctionalUnitType::FMUL, "Float Multiplier", i+1);
        else if (!strcmp(v,"FSTORE")) 
          fu = new FunctionalUnit(FunctionalUnitType::FSTORE, "Float Store", i+1);
        if (!fu) {
          fprintf(stderr,"Error: unknown functional unit (%s).quitting\n",
                  v);
          exit(1);
...
int McOpteron::readNewIMixFile(string filename)
{
  ...
  while (!feof(inf)) {
    //r = fgets(line, sizeof(line), inf); //Scoggin: unused
    ...
    if (!strcmp(mnemonic,"TOTAL"))
      break;
    //fprintf(stderr, "Reading and Looking for: (%s %s %s %s %u)\n", mnemonic, op1, op2, op3, iOpSize); 
    ...
    //fprintf(stderr, "I have: (%s %s %s %s %u)\n", mnemonic, cop1, cop2, cop3, iOpSize); 
    // have all data, add it to a record
    // Waleed: to do this, find the exact instruction using the mnemonic, iOpsize, and operands!
...
int McOpteron::finish(bool printInstMix)
{
  ...
  fprintf(stdout, "***********************\n");
  fprintf(stdout, "Distribution of Cycles:\n");
  fprintf(stdout, "***********************\n");
  fprintf(stdout, "Front End (Fetcher/Decoder Empty Cycles): \n");
  fprintf(stdout, "ITLB Miss Cycles     : %.0f\n", cpiStack->iTLBMiss);
  fprintf(stdout, "ICache Miss Cycles   : %.0f\n", cpiStack->iCacheMiss);
  fprintf(stdout, "Branch Miss Cycles   : %.0f\n", cpiStack->brMiss);
  fprintf(stdout, "Alloc(decode stalls) : %.0f\n", cpiStack->decodeStall);
  fprintf(stdout, "Front End (Decoder not decoding maximum # of instructions):\n");
  fprintf(stdout, "Fetch                : %.0f\n", cpiStack->fetchBuff);
  // update decode
  cpiStack->decode = cpiStack->decode - cpiStack->fetchBuff; 
  fprintf(stdout, "Decode               : %.0f\n", cpiStack->decode);
  fprintf(stdout, "Back End:\n");
  fprintf(stdout, "ROB Empty            : %.0f\n", cpiStack->ROBEmpty);
  fprintf(stdout, "ROB can't retire     : %.0f\n", cpiStack->ROBNotReady);
  fprintf(stdout, "ROB <Max Retire Width: %.0f\n", cpiStack->ROBNotFull);
  fprintf(stdout, "Back  End  Total     : %.0f\n", cpiStack->totalBackend);
  fprintf(stdout, "Back End: Cycle where ROB head instruction can't retire:\n");
  fprintf(stdout, "Instr Latency Cycles: %.0f\n", cpiStack->insLatency);
  fprintf(stdout, "True Data Dependence: %.0f\n", cpiStack->dataDep);
  fprintf(stdout, "Data TLB Miss Cycles: %.0f\n", cpiStack->dTLBMiss);
  fprintf(stdout, "Accessing  Cache  L1: %.0f\n", cpiStack->L1);
  fprintf(stdout, "Accessing  Cache  L2: %.0f\n", cpiStack->L2);
  fprintf(stdout, "Accessing  Cache  L3: %.0f\n", cpiStack->L3);
  fprintf(stdout, "Accessing  Memory   : %.0f\n", cpiStack->mem);
  fprintf(stdout, "Total               : %.0f\n", cpiStack->mem+cpiStack->L3+cpiStack->L2+cpiStack->L1+cpiStack->dTLBMiss+cpiStack->dataDep+cpiStack->insLatency);
  fprintf(stdout, "\n");

  // measure contributions to ROB empty from the front end
  // the following is total number of cycles of events that prevent the decoder from generating anything (decoder empty or ROB empty)
  double totalf = cpiStack->iTLBMiss + cpiStack->iCacheMiss + cpiStack->brMiss + cpiStack->decodeStall; 
  // now, get the percentage of each contributor and multipy times the ROB empty cycles to get the contribution of each
  double c_iTLBMiss    = (cpiStack->ROBEmpty) * cpiStack->iTLBMiss/totalf; 
  double c_iCacheMiss  = (cpiStack->ROBEmpty) * cpiStack->iCacheMiss/totalf; 
  double c_brMiss      = (cpiStack->ROBEmpty) * cpiStack->brMiss/totalf; 
  double c_decodeStall = (cpiStack->ROBEmpty) * cpiStack->decodeStall/totalf; 

  // measure contributions to ROB not being able to retire at max committ width
  // the following is total number of cycles of events that prevent the decoder from decoding at maximum decode width, 
  // which in turn causes the ROB not to retire the max number of instructions per cycle
  double totalf2 = cpiStack->fetchBuff + cpiStack->decode; 
  // now, get the percentage of each contributor and multipy times the ROBNotFull cycles to get the contribution of each
  double c_fetchBuff    = (cpiStack->ROBNotFull) * cpiStack->fetchBuff/totalf2; 
  double c_decode       = (cpiStack->ROBNotFull) * cpiStack->decode/totalf2; 


  // measure contributions to ROB not being able to retire anything!!
  // in the following we measure the contribution of event event causing the ROB not to be able to retire the instruction at the head of ROB
  // there are many events that cause that as follows
  double totalf3 = cpiStack->ROBNotReady; 
  // now, get the percentage of each contributor and multipy times the ROBNotFull cycles to get the contribution of each
  //double c_insLatency    = (cpiStack->ROBNotReady) * cpiStack->insLatency/totalf3; //Scoggin: unused
  double c_dataDep       = (cpiStack->ROBNotReady) * cpiStack->dataDep/totalf3; 
  double c_dTLBMiss      = (cpiStack->ROBNotReady) * cpiStack->dTLBMiss/totalf3; 
  double c_L1            = (cpiStack->ROBNotReady) * cpiStack->L1/totalf3; 
  double c_L2            = (cpiStack->ROBNotReady) * cpiStack->L2/totalf3; 
  double c_L3            = (cpiStack->ROBNotReady) * cpiStack->L3/totalf3; 
  double c_mem           = (cpiStack->ROBNotReady) * cpiStack->mem/totalf3;

  // now output the CPI stack
  double lostCycles = cpiStack->ROBEmpty + cpiStack->ROBNotReady + cpiStack->ROBNotFull; 
  double stackCpi   = (double) 1/3 + (double)(lostCycles)/totalInstructions; 
  fprintf(stdout, "***********************\n");
  fprintf(stdout, "CPI Stacks:\n");
  fprintf(stdout, "***********************\n");
  fprintf(stdout, "%-25s\t%-25s\t%-25s\n", "component", "cycles", "contribution to total CPI");
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Ideal  CPI", (currentCycle-lostCycles) , (double) 1/3); //(currentCycle-lostCycles)/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "iTLB   Miss", c_iTLBMiss , c_iTLBMiss/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "iCache Miss", c_iCacheMiss , c_iCacheMiss/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Branch Misprediction", c_brMiss , c_brMiss/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Decode Stalls", c_decodeStall , c_decodeStall/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Insufficient  Fetch", c_fetchBuff , c_fetchBuff/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Inefficient  Decode", c_decode , c_decode/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "True Data Dependence", c_dataDep , c_dataDep/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "dTLB   Miss", c_dTLBMiss , c_dTLBMiss/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "L1 Cache Access", c_L1 , c_L1/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "L2 Cache Access", c_L2 , c_L2/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "L3 Cache Access", c_L3 , c_L3/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Memory   Access", c_mem , c_mem/totalInstructions);
  fprintf(stdout, "%-25s\t%-25.0f\t%-25.6f\n", "Total", (double)currentCycle , stackCpi);	
  fprintf(stdout, "\n\n\n");	  
...
int McOpteron::simCycle()
{
  ...
  if (Debug>=2) fprintf(stderr, "\n\n======= Simulating cycle %llu ====== \n\n",
                        currentCycle);
  //bool brMispredicted = false; //Scoggin: Unused
  ...
  if(reorderBuffer->updateStatus(currentCycle)){
    ...
    //brMispredicted = true; //Scoggin: Unused
    // empty fake ROB
    ...
    // count this cost
    cpiStack->brMiss +=branchMissPenalty;
...
int McOpteron::fetchInstructions()
{
  ...
  if(fetchedBuffer[0]== NULL) {
    if (nextAvailableFetch > currentCycle) {
      ...
      nextAvailableFetch = memoryModel->serveILoad(currentCycle+1,0,16, cpiStack);
      ...
      if(instructionSizeProbabilities) {
        int curBytes = 0; 
        while((curBytes += generateInstrSize()) <=fetchBuffSize) 
          fetchedInsns++; 
      }
...
int McOpteron::decodeInstructions()
{
  ...
  if(decodeBuffer[0]!= NULL) {
    ...
    cpiStack->decodeStall +=1.0; 
    return 0; 
  }   

  // if fetchedBuffer does not contain at least 'decodeWidth' instructions, count this cycle 
  for(int i=0;i<decodeWidth; i++) { 
    if(!fetchedBuffer[i]) { 
      cpiStack->fetchBuff += (double)(decodeWidth-i)/decodeWidth; 
      break; 
    }
  }   
  ...
  if(numDispatched < decodeWidth) // count cycles at which we don't decdoe max # of isntr
    cpiStack->decode += (double)(decodeWidth-numDispatched)/decodeWidth; 
...
///
/// Allow queues to get new instructions in them if they
/// have room. Instructions are dispatched in order. 
/// This function smartly finds the best combination of available queues
///
int McOpteron::dispatchInstructions()
{
  ...
  // NOTE: the following code is hardcoded for upto 5 decodeWidth only  (maxDispWidth)
  int maxDispWidth = 5; 
  assert(decodeWidth<=5 && maxDispWidth>=numCanAssign); // avoid any problems

  InstructionQueue *possibleQueues[maxDispWidth][numInstrQueues];
  // initilize the array
  for(int i=0; i<maxDispWidth; i++) { 
    for(int i2=0; i2<numInstrQueues; i2++)
      possibleQueues[i][i2] = NULL; 
  } 

  // For every isntruction in decode buffer find all queues that it can be issued to
  // the next step will be to find the best combination of IQ's to allow us to dispatch the maximum number of instructions   
  ...
  // keep track of number of times each queue was assigned an instruction
  std::map<InstructionQueue *,unsigned int> assigned;
  for(int i=0; i<numCanAssign; i++) { 
    for(int q=0; q<numInstrQueues; q++) { 
      if(possibleQueues[i][q] != NULL) 
        assigned[possibleQueues[i][q]] = 0; 
    }  
  }
  // is the ith instruction in the decode queue assignable?
  bool assignable[maxDispWidth];  // ideally, this should be the size of whatever numCanAssign, but restricted to maxDispWidth
  // a running record of target queues
  InstructionQueue *target[maxDispWidth]; // ideally, again this should be the size of whatever numCanAssign 
  // a final record of target queues
  InstructionQueue *finalTargets[maxDispWidth]; // ideally, again this should be the size of whatever numCanAssign
  // initialize
  for(int i=0; i<maxDispWidth; i++){ 
    assignable[i] = false;
    target[i] = finalTargets[i] = NULL; 
  } 	 	   
  int maxAssignable=0, numAssignable = 0; 

  // because it's hard to code the dispatching for arbitrary dispatchWidth, we are putting a restriction of the width for now.
  // the limitation is on the maxAssignable can not be > maxDispWidth (ie decode/dispatch width can not exceed 5)
  for(int q0=0; q0<numInstrQueues; q0++) { // level 1
    if(possibleQueues[0][q0] != NULL) { 
      assignable[0] = true; 
      target[0] = possibleQueues[0][q0];
      assigned[possibleQueues[0][q0]]++; 
      for(int q1=0; q1<numInstrQueues; q1++) { // level 2
        if(possibleQueues[1][q1] != NULL && assigned[possibleQueues[1][q1]] < possibleQueues[1][q1]->getAcceptRate()){
          assignable[1] = true; 
          target[1] = possibleQueues[1][q1];
          assigned[possibleQueues[1][q1]]++;
        }
        for(int q2=0; q2<numInstrQueues; q2++) { // level 3
          if( target[1] && possibleQueues[2][q2] != NULL && assigned[possibleQueues[2][q2]] < possibleQueues[2][q2]->getAcceptRate() ) { 
            assignable[2] = true; 
            target[2] = possibleQueues[2][q2];
            assigned[possibleQueues[2][q2]]++;
          }
          for(int q3=0; q3<numInstrQueues; q3++) { // level 4
            if( target[2] && possibleQueues[3][q3] != NULL && assigned[possibleQueues[3][q3]] < possibleQueues[3][q3]->getAcceptRate() ) { 
              assignable[3] = true; 
              target[3] = possibleQueues[3][q3];
              assigned[possibleQueues[3][q3]]++;
            }
            for(int q4=0; q4<numInstrQueues; q4++) { // level 5
              if(target[3] && possibleQueues[4][q4] != NULL && assigned[possibleQueues[4][q4]] < possibleQueues[4][q4]->getAcceptRate() ) { 
                assignable[4] = true; 
                target[4] = possibleQueues[4][q4];
                assigned[possibleQueues[4][q4]]++;						 				
              }
              // now we check what we have
              // how many can we assign at this iteration
              for(int a=0; a<numCanAssign; a++) { if(assignable[a]) numAssignable++; }			
              // remember best case
              ...
              numAssignable = 0;
              // some keeping, reset variables at end of every level loop
              if(target[4] ==  possibleQueues[4][q4]) assigned[possibleQueues[4][q4]]--;
              target[4] = NULL;
              assignable[4] = false; 
            } // end level 5 loop
            // some keeping, reset variables at end of every level loop
            if(target[3] ==  possibleQueues[3][q3]) assigned[possibleQueues[3][q3]]--;
            target[3] = NULL;
            assignable[3] = false; 
          } // end level 4 loop
          // some keeping, reset variables at end of every level loop
          if(target[2] ==  possibleQueues[2][q2]) assigned[possibleQueues[2][q2]]--;
          target[2] = NULL;
          assignable[2] = false; 
        } // end level 3 loop
        // some keeping, reset variables at end of every level loop
        if(target[1] ==  possibleQueues[1][q1]) assigned[possibleQueues[1][q1]]--;
        target[1] = NULL;
        assignable[1] = false; 
      } // end level 2 loop
    } 
    ...
    if(maxAssignable==numCanAssign)
      break; 
    // reset numbers
    for(int i=0; i<maxDispWidth; ++i) { target[i] = NULL; assignable[i] = false; } 
    std::map<InstructionQueue *,unsigned int>::iterator it = assigned.begin();
    for(; it!=assigned.end(); ++it) assigned[it->first] = 0;  
  ...
  // how many instr were dispatched at the end?
  if (Debug>=2) fprintf(stderr, "Refilling instruction queues: %d tokens were dispatched at this cycle\n", numAssigned);

#if 0 
  if(numAssigned < decodeWidth) { // count cycles were we did not dispatch max num of instr
    //cpiStack->dispatch += (double)(decodeWidth-numAssigned)/decodeWidth; 
    //cpiStack->decodeStall +=(double)(decodeWidth-numAssigned)/decodeWidth;
  }
#endif
...
Token* McOpteron::generateToken()
{
  ...
  // create token
  t = new Token(type, ++totalInstructions, currentCycle, false, cpiStack);
...
Token* McOpteron::getNextTraceToken()
{
  ...
  t = new Token(it, ++totalInstructions, currentCycle, false, cpiStack);
  // set source ops
  ...
  for(unsigned int i=0; i<sourceOps; i++) { 
    if (!fgets(line, sizeof(line), traceF)) {
      if (repeatTrace) {
        fseek(traceF,0,SEEK_SET);
        if (!fgets(line, sizeof(line), traceF)){
          fprintf(stderr,"error capturing distances\n");
          delete[] distances;
          return 0;
        }
      } else{
        fprintf(stderr,"second error capturing distances\n");
        delete[] distances;
        return 0;
      }
    }
    int n = sscanf(line, "%u", &distances[i]);
    if (n != 1)
      return 0;
  }   
...
int main(int argc, char **argv)
{
  ...
  unsigned long numSimCycles = 500000;
  ...
  string transFile;
  //bool useNewIMix = false; //Scoggin: Never Used
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp("--debug",argv[i])) {
      ...
      if (i == argc-1) doHelp();
      newIMixFile = argv[++i];
      //useNewIMix = true; //Scoggin: Never Used
