...
#include "pse1x2multiring.h"

Define_Module(PSE1x2MultiRing);

PSE1x2MultiRing::PSE1x2MultiRing()
{
}
...
double PSE1x2MultiRing::GetLatency(int indexIn, int indexOut)
{
	return latencyMatrix[indexIn][indexOut];
}

double PSE1x2MultiRing::GetPropagationLoss(int indexIn, int indexOut, double wavelength)
{
...
double PSE1x2MultiRing::GetBendingLoss(int indexIn, int indexOut, double wavelength)
{
	int ringSet;
	bool resonant = IsInResonance(wavelength,ringSet);
...
double PSE1x2MultiRing::GetCrossingLoss(int indexIn, int indexOut, double wavelength)
{
	if(indexIn != indexOut)
	{
...
double PSE1x2MultiRing::GetDropIntoRingLoss(int indexIn, int indexOut, double wavelength)
{
	double IL = 0;
	int ringSet;
	bool resonant = IsInResonance(wavelength,ringSet);
...
double PSE1x2MultiRing::GetPassByRingLoss(int indexIn, int indexOut, double wavelength)
{
	double IL = 0;
	int ringSet;
	bool resonant = IsInResonance(wavelength,ringSet);
...
double PSE1x2MultiRing::GetPowerLevel(int state)
{
	switch(state)
	{
	case 0:
...
double PSE1x2MultiRing::GetEnergyDissipation(int stateBefore, int stateAfter)
{
...
void PSE1x2MultiRing::SetRoutingTable()
{
	// off resonance case
	routingTable[0] = 2;
	routingTable[1] = 3;
...
int PSE1x2MultiRing::GetMultiRingRoutingTable(int index, int ringSet)
{
